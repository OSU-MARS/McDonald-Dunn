...
if (otherOptions$fitHeight)
{
  ...
  #otherHeightFromDiameter$chapmanRichardsBalPhysioRelDbh = fit_gsl_nls("Chapman-Richards RelDbh BA+L physio", height ~ 1.37 + (a1 + a1bal * basalAreaLarger + a1rd * relativeDiameter + a1e * elevation) * (1 - exp(b1 * dbh))^b2, other2022, start = list(a1 = 80, a1bal = 0.3, a1rd = 0, a1e = 0, b1 = -0.01, b2 = 0.8), significant = FALSE) # by propagation
  ...
  # reciprocal BA+L
  otherHeightFromDiameter$chapmanRichardsBal = fit_gsl_nls("Chapman-Richards BA+L", height ~ 1.37 + (a1 + a1bal / (a1balr + basalAreaLarger)) * (1 - exp((b1) * dbh))^(b2), other2022, start = list(a1 = 30, a1bal = 0, a1balr = 20, b1 = -0.02, b2 = 0.84), significant = FALSE) # a1ba, a1ba+r, a1bal+r, b1ba, b1ba+r, b2ba, b2ba+r, b2bal+r not significant, b1bal+r NaN-Inf
  otherHeightFromDiameter$chapmanRichardsBalPhysio = fit_gsl_nls("Chapman-Richards BA+L physio", height ~ 1.37 + (a1 + a1bal / (a1balr + basalAreaLarger) + a1tw * topographicWetnessFD8f) * (1 - exp(b1 * dbh))^b2, other2022, start = list(a1 = 30, a1bal = 0, a1balr = 20, a1tw = 0, b1 = -0.02, b2 = 0.8), significant = FALSE) # by propagation, a1tw+e not significant
  otherHeightFromDiameter$chapmanRichardsBalPhysioRelDbh = fit_gsl_nls("Chapman-Richards RelDbh BA+L physio", height ~ 1.37 + (a1 + a1bal / (a1balr + basalAreaLarger) + a1rd * relativeDiameter + a1e * elevation) * (1 - exp(b1 * dbh))^b2, other2022, start = list(a1 = 80, a1bal = 0, a1balr = 20, a1rd = 0, a1e = 0, b1 = -0.01, b2 = 0.8), significant = FALSE) # by propagation
  otherHeightFromDiameter$chapmanRichardsBalRelDbh = fit_gsl_nls("Chapman-Richards RelDbh BA+L", height ~ 1.37 + (a1 + a1bal / (a1balr + basalAreaLarger) + a1rd * relativeDiameter) * (1 - exp(b1 * dbh))^b2, other2022, start = list(a1 = 30, a1bal = 0, a1balr = 20, a1rd = 0, b1 = -0.02, b2 = 0.84), significant = FALSE) # by propagation
  ...
  otherHeightFromDiameter$sharmaPartonPhysio = fit_gsl_nls("Sharma-Parton physio", height ~ 1.37 + (a1 + a1tw * topographicWetnessFD8f)*topHeight^(b1) * (1 - exp((b2)*(standTreesPerHectare/(standBasalAreaPerHectare))^(b3)*dbh))^(b4), other2022, start = list(a1 = 5, a1tw = 0, b1 = 0.5, b2 = -0.01, b3 = 0.3, b4 = 1.0), significant = FALSE) # { a1, b1, b2, b3, b4 } x { e, s, a, tr, tw }, a1tw+e not significant
  ...
  #otherHeightFromDiameter$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", height ~ 1.37 + (a1)*standBasalAreaPerHectare^(b1) * (1 - exp((b2)*standTreesPerHectare^(b3)*dbh))^(b4 + b4bal / standBasalAreaPerHectare), other2022, start = list(a1 = 25, b1 = 0.05, b2 = -0.01, b3 = 0.2, b4 = 0.9, b4bal = 0), significant = FALSE) # a1ba, a1ba+r, b1ba, b1ba+r, b3ba, b3ba+r, b4ba, b4ba+r, b4bal+r not significant, a1bal+r, b1bal+r, b2ba, b2bal+r, b3bal+r NaN-Inf
  ...
  otherHeightFromDiameter$weibullBal = fit_gsl_nls("Weibull BA+L", height ~ 1.37 + (a1) * (1 - exp((b1 + b1bal / standBasalAreaPerHectare) * dbh^(b2))), other2022, start = list(a1 = 40, b1 = -0.04, b1bal = 0, b2 = 0.9), significant = FALSE) # { a1, b1, b2 } x { ba, ba+r, bal+r } not significant
  ...
  otherHeightFromDiameter$gamBalPhysioRelDbh = fit_gam("REML GAM RelDbh BA+L physio", height ~ I(1.523 * dbh^0.697) + s(dbh, basalAreaLarger, terrainRoughness, relativeDiameter, bs = "ts", by = as.factor(isPlantation), k = 16), data = other2022, significant = FALSE) # by propagation
...
if (otherOptions$fitHeightMixed)
{
  ...
  otherHeightFromDiameterMixed$chapmanRichardsBalPhysioRelDbh = create_fit_statistics("Chapman-Richards RelDbh BA+L physio", fitting = "nlme", significant = FALSE)
  otherHeightFromDiameterMixed$chapmanRichardsBalRelDbh = create_fit_statistics("Chapman-Richards RelDbh BA+L", fitting = "nlme", significant = FALSE)
  ...
  otherHeightFromDiameterMixed$sharmaPartonBal = create_fit_statistics(name = "Sharma-Parton BA+L", fitting = "nlme", significant = FALSE)
  ...
  otherHeightFromDiameterMixed$sharmaPartonBalPhysio = create_fit_statistics(name = "Sharma-Parton BA+L physio", fitting = "nlme", significant = FALSE)
  ...
  otherHeightFromDiameterMixed$sharmaPartonBalPhysioRelDbh = create_fit_statistics(name = "Sharma-Parton RelDbh BA+L physio", fitting = "nlme", significant = FALSE)
  ...
  otherHeightFromDiameterMixed$sharmaPartonBalRelDbh = create_fit_statistics(name = "Sharma-Parton RelDbh BA+L", fitting = "nlme", significant = FALSE)
  ...
  otherHeightFromDiameterMixed$weibullBal = create_fit_statistics("Weibull BA+L", fitting = "nlme", significant = FALSE)
  ...
  otherHeightFromDiameterMixed$gamBalPhysioRelDbh = fit_gam("REML GAM RelDbh BA+L physio", height ~ I(1.523 * dbh^0.697) + s(dbh, basalAreaLarger, terrainRoughness, relativeDiameter, bs = "ts", by = as.factor(isPlantation), k = 16), random = list(uniquePlotID = ~1), data = other2022, significant = FALSE)
  otherHeightFromDiameterMixed$gamBalRelDbh = fit_gam("REML GAM RelDbh BA+L", height ~ I(1.523 * dbh^0.697) + s(dbh, basalAreaLarger, relativeDiameter, bs = "ts", k = 11), random = list(uniquePlotID = ~1), data = other2022, significant = FALSE) # k = 11 minimum> ~6
