...
if (acmaOptions$fitHeight)
{
  ...
  acmaHeightFromDiameter$chapmanRichards = fit_gsl_nls("Chapman-Richards", height ~ 1.37 + a1*(1 - exp(b1*dbh))^b2, acma2022, start = list(a1 = 27, b1 = -0.06, b2 = 1.1, b2p = -0.2)) # a1p, b1p, b2p not significant
  acmaHeightFromDiameter$chapmanRichardsBal = fit_gsl_nls("Chapman-Richards BA+L", height ~ 1.37 + (a1 + a2 * basalAreaLarger) * (1 - exp(b1*dbh))^b2, acma2022, start = list(a1 = 27, a2 = 0, b1 = -0.06, b2 = 1.1), significant = FALSE) # a2, a3, b1ba, b1bal, b2ba, b2bal not significant, a2, b1bal significant
  ...
  acmaHeightFromDiameter$chapmanRichardsBalRelDbh = fit_gsl_nls("Chapman-Richards BA+L RelDbh", height ~ 1.37 + (a1 + a2 * basalAreaLarger + a9 * relativeDiameter) * (1 - exp(b1*dbh))^(b2 + b2p * isPlantation), acma2022, start = list(a1 = 27, a2 = 0, a9 = 0, b1 = -0.027, b2 = 1.08, b2p = -0.2), significant = FALSE) # a2, a9 not significant
  ...
  acmaHeightFromDiameter$chapmanRichardsRelDbh = fit_gsl_nls("Chapman-Richards RelDbh", height ~ 1.37 + (a1 + a9 * relativeDiameter)*(1 - exp(b1*dbh))^b2, acma2022, start = list(a1 = 27, a9 = 0, b1 = -0.06, b2 = 1.1), significant = FALSE) # a9, b2p, b2rd not significant, b1rd nan-inf
  ...
  acmaHeightFromDiameter$korf = fit_gsl_nls("Korf", height ~ 1.37 + a1*exp(b1*dbh^b2), acma2022, start = list(a1 = 70, b1 = -5.0, b2 = -0.35, b2p = 0)) # a1p, b1p, b2p not signficant
  ...
  acmaHeightFromDiameter$richardsW = fit_gsl_nls("unified Richards", height ~ 1.37 + Ha * (1 + ((1.37/Ha)^(1 - d) - 1) * exp((-kU * dbh)/d^(d/(1 - d))))^(1/(1 - d)), acma2022, start = list(Ha = 25, d = 0.7, kU = 0.035)) # Hap, kUp, dp not signficant
  ...
  acmaHeightFromDiameter$sharmaPartonBal = fit_gsl_nls("Sharma-Parton BA+L", height ~ 1.37 + a1*topHeight^b1 * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^b3*dbh))^(b4 + b4p * isPlantation), acma2022, start = list(a1 = 15, b1 = 0.14, b2 = -0.03, b3 = -0.06, b4 = 1.1, b4p = -0.24))
  to_fixed_coeffficients(acmaHeightFromDiameter$sharmaPartonBal)
  print(to_parameter_confidence_intervals(acmaHeightFromDiameter$sharmaPartonBal), n = 8)
  acmaHeightFromDiameter$sharmaPartonBalPhysio = fit_gsl_nls("Sharma-Parton BA+L physio", height ~ 1.37 + (a1 + a6 * sin(3.14159/180 * aspect))*topHeight^(b1 + b1p * isPlantation) * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^b3*dbh))^b4, acma2022, start = list(a1 = 12, a6 = 0.4, b1 = 0.2, b1p = 0.04, b2 = -0.037, b3 = -0.046, b4 = 1.06), control = gsl_nls_control())
  acmaHeightFromDiameter$sharmaPartonBalPhysioRelDbh = fit_gsl_nls("Sharma-Parton BA+L RelDbh physio", height ~ 1.37 + (a1 + a6 * sin(3.14159/180 * aspect) + a9 * relativeDiameter)*topHeight^(b1 + b1p * isPlantation) * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^b3*dbh))^b4, acma2022, start = list(a1 = 13, a6 = 0, a9 = -0.5, b1 = 0.2, b1p = 0.05, b2 = -0.024, b3 = 0.06, b4 = 1.0))
  acmaHeightFromDiameter$sharmaPartonBalRelDbh = fit_gsl_nls("Sharma-Parton BA+L RelDbh", height ~ 1.37 + (a1 + a9 * relativeDiameter)*topHeight^b1 * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^b3*dbh))^(b4 + b4p * isPlantation), acma2022, start = list(a1 = 16, a9 = -0.5, b1 = 0.14, b2 = -0.026, b3 = 0.06, b4 = 1.12, b4p = -0.24))
  acmaHeightFromDiameter$sharmaPartonPhysio = fit_gsl_nls("Sharma-Parton physio", height ~ 1.37 + (a1 + a6 * sin(3.14159/180 * aspect))*topHeight^(b1 + b1p * isPlantation) * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare))^b3*dbh))^b4, acma2022, start = list(a1 = 11, a6 = 0.3, b1 = 0.2, b1p = 0.06, b2 = -0.03, b3 = 0, b4 = 1.0))
  acmaHeightFromDiameter$sharmaPartonRelDbh = fit_gsl_nls("Sharma-Parton RelDbh", height ~ 1.37 + (a1 + a9 * relativeDiameter)*topHeight^b1*(1 - exp(b2*(standTreesPerHectare/standBasalAreaPerHectare)^b3*dbh))^(b4 + b4p * isPlantation), acma2022, start = list(a1 = 17, a9 = -0.6, b1 = 0.14, b2 = -0.03, b3 = 0.08, b4 = 1.1, b4p = -0.23))
  acmaHeightFromDiameter$sharmaPartonRelDbhPhysio = fit_gsl_nls("Sharma-Parton RelDbh physio", height ~ 1.37 + (a1 + a6 * sin(3.14159/180 * aspect) + a9 * relativeDiameter)*topHeight^(b1 + b1p * isPlantation) * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare))^b3*dbh))^b4, acma2022, start = list(a1 = 11, a6 = 0.3, a9 = -0.6, b1 = 0.23, b1p = 0.05, b2 = -0.02, b3 = 0.10, b4 = 0.96))
  acmaHeightFromDiameter$sharmaZhang = fit_gsl_nls("Sharma-Zhang", height ~ 1.37 + (a1 + a1p * isPlantation)*standBasalAreaPerHectare^b1*(1 - exp(b2*standTreesPerHectare^b3*dbh))^b4, acma2022, start = list(a1 = 17, a1p = 2.0, b1 = 0.1, b2 = -0.05, b3 = -0.1, b4 = 1.0))
  acmaHeightFromDiameter$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", height ~ 1.37 + (a1 + a2 * basalAreaLarger)*standBasalAreaPerHectare^b1 * (1 - exp(b2*standTreesPerHectare^b3*dbh))^(b4 + b4p * isPlantation), acma2022, start = list(a1 = 22, a2 = 0.05, b1 = 0.05, b2 = -0.02, b3 = 0.04, b4 = 1.09, b4p = -0.2))
...
if (acmaOptions$fitHeightMixed)
{
  ...
  acmaHeightFromDiameterMixed$curtis = fit_nlme("Curtis", height ~ 1.37 + (a1 + a1p * isPlantation + a1r) * dbh / (1 + dbh)^b1, acma2022, 
                                                fixedFormula = a1 + a1p + b1 ~ 1, randomFormula = a1r ~ 1|stand/plot,
                                                start = list(fixed = c(a1 = 1.24, a1p = 0.23, b1 = 0.28)), control = nlmeControl())
  acmaHeightFromDiameterMixed$hossfeld = fit_nlme("Hossfeld IV", height ~ 1.37 + (a1 + a1p * isPlantation + a1r) / (1 + a2 * dbh^b1), acma2022, 
  ...
  acmaHeightFromDiameterMixed$prodan = fit_nlme("Prodan", height ~ 1.37 + dbh^2 / (a1 * dbh^2 + (a2 + a2p * isPlantation)*dbh + a3 + a3r), acma2022, 
                                                fixedFormula = a1 + a2 + a2p + a3 ~ 1, randomFormula = a3r ~ 1|stand/plot,
  ...
  acmaHeightFromDiameterMixed$richardsW = fit_nlme("unified Richards", height ~ 1.37 + (Ha + Har) * (1 + ((1.37/(Ha + Har))^(1 - d) - 1) * exp((-(kU + kUp * isPlantation) * dbh)/d^(d/(1 - d))))^(1/(1 - d)), acma2022, 
                                                   fixedFormula = Ha + d + kU + kUp ~ 1, randomFormula = Har ~ 1|stand/plot,
                                                   start = list(fixed = c(Ha = 22.8, d = 0.723, kU = 0.025, kUp = 0.0064)))
  acmaHeightFromDiameterMixed$sharmaParton = fit_nlme("Sharma-Parton", height ~ 1.37 + (a1 + a1r)*topHeight^b1*(1 - exp(b2*(standTreesPerHectare/standBasalAreaPerHectare)^b3*dbh))^(b4 + b4p * isPlantation), acma2022, 
                                                      fixedFormula = a1 + b1 + b2 + b3 + b4 + b4p ~ 1, randomFormula = a1r ~ 1|stand/plot,
                                                      start = list(fixed = c(a1 = 19, b1 = 0.1, b2 = -0.03, b3 = 0.06, b4 = 1.1, b4p = -0.25)), control = nlmeControl())
  acmaHeightFromDiameterMixed$sharmaPartonBal = fit_nlme("Sharma-Parton BA+L", height ~ 1.37 + (a1 + a1r)*topHeight^b1 * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^b3*dbh))^(b4 + b4p * isPlantation), acma2022,
                                                         fixedFormula = a1 + b1 + b2 + b3 + b4 + b4p ~ 1, randomFormula = a1r ~ 1|stand/plot,
                                                         start = list(fixed = c(a1 = 15, b1 = 0.14, b2 = -0.03, b3 = -0.06, b4 = 1.1, b4p = -0.24)), control = nlmeControl())
  acmaHeightFromDiameterMixed$sharmaPartonBalPhysio = fit_nlme("Sharma-Parton BA+L physio", height ~ 1.37 + (a1 + a1r + a6 * sin(3.14159/180 * aspect))*topHeight^(b1 + b1p * isPlantation) * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^b3*dbh))^b4, acma2022, 
  ...
  acmaHeightFromDiameterMixed$sharmaZhang = fit_nlme("Sharma-Zhang", height ~ 1.37 + (a1 + a1p * isPlantation + a1r)*standBasalAreaPerHectare^b1*(1 - exp(b2*standTreesPerHectare^b3*dbh))^b4, acma2022,
                                                     fixedFormula = a1 + a1p + b1 + b2 + b3 + b4 ~ 1, randomFormula = a1r ~ 1|stand/plot,
                                                     start = list(fixed = c(a1 = 17, a1p = 2.0, b1 = 0.1, b2 = -0.05, b3 = -0.1, b4 = 1.0)), control = nlmeControl())
  acmaHeightFromDiameterMixed$sharmaZhangBal = fit_nlme("Sharma-Zhang BA+L", height ~ 1.37 + (a1 + a1r + a2 * basalAreaLarger)*standBasalAreaPerHectare^b1 * (1 - exp(b2*standTreesPerHectare^b3*dbh))^(b4 + b4p * isPlantation), acma2022, 
                                                        fixedFormula = a1 + a2 + b1 + b2 + b3 + b4 + b4p ~ 1, randomFormula = a1r ~ 1|stand/plot,
...
if (acmaOptions$fitDbh)
{
  ...
  acmaDiameterFromHeight$sharmaParton = fit_gsl_nls("modified Sharma-Parton", dbh ~ a1*(height - 1.37)^b1*(exp(b2*(standTreesPerHectare/topHeight)^b3*(height - 1.37)) - 1)^b4, acma2022, start = list(a1 = 0.29, b1 = 1.14, b2 = 0.00001, b3 = 0.8, b4 = -0.2), control = gsl_nls_control())
...
if (acmaOptions$fitDbhMixed)
{
  ...
  acmaDiameterFromHeightMixed = list(chapmanReplaceBalRelHt = fit_nlme("Chapman-Richards replace BA+L RelHt", dbh ~ (a1 + a1r + a2 * basalAreaLarger + a3 * standBasalAreaPerHectare + a9 * relativeHeight) * (exp(b1*(height - 1.37)^b2) - 1), acma2022, 
                                                                       fixedFormula = a1 + a2 + a3 + a9 + b1 + b2 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  acmaDiameterFromHeightMixed$powerPhysio = fit_nlme("power physio", dbh ~ (a1 + a1r + a1p * isPlantation + a5 * sin(3.14159/180 * slope) + a8 * terrainRoughness)*(height - 1.37)^b1, acma2022,
                                                     fixedFormula = a1 + a1p + a5 + a8 + b1 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  acmaDiameterFromHeightMixed$ruarkAbatPhysioRelHt = fit_nlme("Ruark ABA+T RelHt physio", dbh ~ (a1 + a1r + a3 * standBasalAreaApprox + a7 * sin(3.14159/180 * aspect) + a9 * relativeHeight)*(height - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (height - 1.37)), acma2022,
                                                              fixedFormula = a1 + a3 + a7 + a9 + b1 + b1 + b1p + b2 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  acmaDiameterFromHeightMixed$ruarkAbatRelHt = fit_nlme("Ruark ABA+T RelHt", dbh ~ (a1 + a1r + a2 * tallerApproxBasalArea + a9 * relativeHeight)*(height - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (height - 1.37)), acma2022, 
                                                        fixedFormula = a1 + a2 + a9 + b1 + b1p + b2 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  acmaDiameterFromHeightMixed$ruarkRelHt = fit_nlme("Ruark RelHt", dbh ~ (a1 + a1r + a9 * relativeHeight)*(height - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (height - 1.37)), acma2022, 
                                                    fixedFormula = a1 + a9 + b1 + b1p + b2 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  acmaDiameterFromHeightMixed$ruarkRelHtPhysio = fit_nlme("Ruark RelHt physio", dbh ~ (a1 + a1r + a7 * sin(3.14159/180 * aspect) + a9 * relativeHeight)*(height - 1.37)^(b1 + b1p * isPlantation) * exp(b2 * (height - 1.37)), acma2022,
                                                          fixedFormula = a1 + a7 + a9 + b1 + b1p + b2 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  acmaDiameterFromHeightMixed$sharmaParton = fit_nlme("modified Sharma-Parton", dbh ~ (a1 + a1r) * (height - 1.37)^b1*(exp(b2*(standTreesPerHectare/topHeight)^b3*(height - 1.37)) - 1)^b4, acma2022, 
                                                      fixedFormula = a1 + b1 + b2 + b3 + b4 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  acmaDiameterFromHeightMixed$sibbesenReplaceAbatPhysioRelHt = fit_nlme("Sibbesen replace ABA+T RelHt physio", dbh ~ (a1 + a1r + a3 * standBasalAreaApprox + a5 * sin(3.14159/180 * slope) + a9 * relativeHeight)*(height - 1.37)^(b1*(height - 1.37)^b2), acma2022, 
                                                                        fixedFormula = a1 + a3 + a5 + a9 + b1 + b2 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  acmaDiameterFromHeightMixed$sibbesenReplaceRelHtPhysio = fit_nlme("Sibbesen replace RelHt physio", dbh ~ (a1 + a1r + a5 * sin(3.14159/180 * slope) + a9 * relativeHeight)*(height - 1.37)^(b1*(height - 1.37)^b2), acma2022,
                                                                    fixedFormula = a1 + a5 + a9 + b1 + b2 ~ 1, randomFormula = a1r ~ 1|stand/plot,
...
if (acmaOptions$fitHeight & acmaOptions$fitDbh)
{
  ...
  acmaHeightFromDiameterPreferred$sharmaParton = fit_gsl_nls("Sharma-Parton", height ~ 1.37 + a1*topHeight^b1*(1 - exp(b2*(standTreesPerHectare/standBasalAreaPerHectare)^b3*dbh))^(b4 + b4p * isPlantation), acma2022, start = list(a1 = 19, b1 = 0.1, b2 = -0.03, b3 = 0.06, b4 = 1.1, b4p = -0.25), folds = 1, repetitions = 1)
