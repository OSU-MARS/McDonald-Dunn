...
if (psmeOptions$fitHeightPrimary)
{
  ...
  psmeHeightFromDiameter$chapmanRichardsBal = fit_gsl_nls("Chapman-Richards BA+L", height ~ 1.37 + (a1 + a2 * standBasalAreaPerHectare) * (1 - exp((b1)*dbh))^(b2 + b2bal * basalAreaLarger), psme2022, start = list(a1 = 57, a2 = 0.1, b1 = -0.02, b2 = 1.3, b2bal = -0.005)) # a2p, b1ba, b1bap, b2p, b2ba, b2bap, b2balp not significant, a3, a3p, b1bal, b1balp also significant
  psmeHeightFromDiameter$chapmanRichardsBalRelDbh = fit_gsl_nls("Chapman-Richards BA+L RelDbh", height ~ 1.37 + (a1 + a2 * standBasalAreaPerHectare) * (1 - exp((b1)*dbh))^(b2 + b2rd * relativeDiameter + b2bal * basalAreaLarger), psme2022, start = list(a1 = 57, a2 = 0.05, b1 = -0.02, b2 = 1.25, b2rd = 0.1, b2bal = -0.005)) # a4p, b1rdp, b2rdp not significant, a4, b1rd also significant
  ...
  psmeHeightFromDiameter$sharmaPartonBalPhysioRelDbh = fit_gsl_nls("Sharma-Parton BA+L RelDbh physio", height ~ 1.37 + (a1 + a4 * relativeDiameter)*topHeight^(b1) * (1 - exp((b2)*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^(b3)*dbh))^(b4 + b4e * elevation + b4as * sin(3.14159/180 * aspect) + b4ac * cos(3.14159/180 * aspect)), psme2022, start = list(a1 = 20, a4 = 0.4, b1 = 0.3, b2 = -0.02, b3 = -0.05, b4 = 1.0, b4e = 0.0001, b4as = -0.04, b4ac = -0.04))
  ...
  psmeHeightFromDiameter$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", height ~ 1.37 + (a1 + a1p * isPlantation)*standBasalAreaPerHectare^(b1 + b1p * isPlantation) * (1 - exp((b2)*standTreesPerHectare^b3 * dbh))^(b4 + b4ba * standBasalAreaPerHectare + b4bal * basalAreaLarger), psme2022, start = list(a1 = 30, a1p = -10, b1 = 0.1, b1p = 0.1, b2 = -0.03, b3 = -0.02, b4 = 1.1, b4ba = 0.01, b4bal = -0.01)) # a2p, a3p, { b1, b3 } x { bal, balp }, b4bap, b4balp not significant, b3 not significant, a2, a3, b1ba also significant, 10x10 AIC 3779
  #psmeHeightFromDiameter$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", height ~ 1.37 + (a1 + a1p * isPlantation)*(standBasalAreaPerHectare + basalAreaLarger)^(b1 + b1p * isPlantation) * (1 - exp(b2*standTreesPerHectare^b3 * dbh))^(b4 + b4ba * standBasalAreaPerHectare + b4bal * basalAreaLarger), psme2022, start = list(a1 = 40, a1p = -10, b1 = 0.1, b1p = 0.1, b2 = -0.03, b3 = -0.04, b4 = 1.0, b4ba = 0.01, b4bal = -0.01)) # b2p, b3p, b4p not significant, 10x10 AIC 3796
  ...
  psmeHeightFromDiameter$gam = fit_gam("REML GAM", height ~ I(dbh^(0.6516 + 0.1398 * isPlantation)) + s(dbh, bs = "ts", by = as.factor(isPlantation), k = 7), data = psme2022, threads = 4) # 10x10 AIC 3886
  #psmeHeightFromDiameter$gamBa = fit_gam("REML GAM BA", height ~ I(dbh^(0.6516 + 0.1398 * isPlantation)) + s(dbh, standBasalAreaPerHectare, bs = "ts", by = as.factor(isPlantation), k = 15), data = psme2022, threads = 4) # 10x10 AIC 3881
  psmeHeightFromDiameter$gamBal = fit_gam("REML GAM BA+L", height ~ I(dbh^(0.6516 + 0.1398 * isPlantation)) + s(dbh, basalAreaLarger, bs = "ts", by = as.factor(isPlantation), k = 16), data = psme2022, threads = 4) # 10x10 AIC 3825
  #psmeHeightFromDiameter$gamBaBal = fit_gam("REML GAM BA+L", height ~ I(dbh^(0.6516 + 0.1398 * isPlantation)) + s(dbh, standBasalAreaPerHectare, basalAreaLarger, bs = "ts", by = as.factor(isPlantation), k = 13), data = psme2022, threads = 4) # 10x10 AIC 3836
  psmeHeightFromDiameter$gamBalRelDbh = fit_gam("REML GAM BA+L RelDbh", height ~ I(dbh^(0.6516 + 0.1398 * isPlantation)) + s(dbh, basalAreaLarger, relativeDiameter, bs = "ts", by = as.factor(isPlantation), k = 13), data = psme2022, threads = 4) # 10x10 AIC 3818
  ...
  {
    psmeHeightFromDiameter$gamBalPhysio = fit_gam("REML GAM BA+L physio", height ~ I(dbh^(0.6516 + 0.1398 * isPlantation)) + s(dbh, basalAreaLarger, topographicWetnessFD8f, bs = "ts", by = as.factor(isPlantation), k = 15), data = psme2022, threads = 4) # 10x10 AIC 3789
    psmeHeightFromDiameter$gamBalPhysioRelDbh = fit_gam("REML GAM BA+L RelDbh physio", height ~ I(dbh^(0.6516 + 0.1398 * isPlantation)) + s(dbh, basalAreaLarger, relativeDiameter, topographicWetnessFD8f, bs = "ts", by = as.factor(isPlantation), k = 19), data = psme2022, threads = 4) # 10x10 AIC 3848
...
if (psmeOptions$fitHeightMixed)
{
  ...
  psmeHeightFromDiameterMixed$chapmanRichardsBalRelDbh = fit_nlme("Chapman-Richards BA+L RelDbh", height ~ 1.37 + (a1 + a1r + a2 * standBasalAreaPerHectare) * (1 - exp(b1*dbh))^(b2 + b2rd * relativeDiameter), psme2022,  # b1r, b2r singularity in backsolve, b2balp not significant
                                                                  fixedFormula = a1 + a2 + b1 + b2 + b2rd ~ 1, randomFormula = a1r ~ 1|stand/plot,
                                                                  start = list(fixed = c(a1 = 40, a2 = 0.2, b1 = -0.03, b2 = 1.2, b2rd = 0.2)))
  ...
  psmeHeightFromDiameterMixed$sharmaPartonBalPhysio = fit_nlme("Sharma-Parton BA+L physio", height ~ 1.37 + a1*topHeight^(b1 + b1r) * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^(b3)*dbh))^(b4 + b4e * elevation + b4as * sin(3.14159/180 * aspect) + b4ac * cos(3.14159/180 * aspect)), psme2022, # a1r, b24, b4r singularity in backsolve, b3r also tractable
                                                               ...
                                                               start = list(fixed = c(a1 = 35, b1 = 0.1, b2 = -0.02, b3 = -0.1, b4 = 1.0, b4e = 0.0002, b4as = -0.035, b4ac = -0.04)))
  psmeHeightFromDiameterMixed$sharmaPartonBalPhysioRelDbh = fit_nlme("Sharma-Parton BA+L RelDbh physio", height ~ 1.37 + (a1 + a4 * relativeDiameter)*topHeight^(b1 + b1r) * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^(b3)*dbh))^(b4 + b4e * elevation + b4as * sin(3.14159/180 * aspect) + b4ac * cos(3.14159/180 * aspect)), psme2022, # a1r, b24, b4r singularity in backsolve, b3r also tractable
                                                                     fixedFormula = a1 + a4 + b1 + b2 + b3 + b4 + b4e + b4as + b4ac ~ 1, randomFormula = b1r ~ 1|stand/plot,
                                                                     start = list(fixed = c(a1 = 30, a4 = 0.4, b1 = 0.1, b2 = -0.02, b3 = -0.1, b4 = 1.0, b4e = 0.0002, b4as = -0.035, b4ac = -0.04)))
  psmeHeightFromDiameterMixed$sharmaPartonBalRelDbh = fit_nlme("Sharma-Parton BA+L RelDbh", height ~ 1.37 + (a1)*topHeight^(b1 + b1r) * (1 - exp((b2)*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^(b3)*dbh))^(b4 + b4rd * relativeDiameter), psme2022, # a4 not significant, b1rd, b2rd, b3rd also tractable
  ...
  psmeHeightFromDiameterMixed$sharmaPartonRelDbhPysio = create_fit_statistics("Sharma-Parton RelDbh physio", fitting = "nlme", significant = FALSE)
  ...
  psmeHeightFromDiameterMixed$sharmaZhangBal = fit_nlme("Sharma-Zhang BA+L", height ~ 1.37 + (a1 + a1p * isPlantation) * standBasalAreaPerHectare^(b1 + b1p * isPlantation + b1r) * (1 - exp(b2 * standTreesPerHectare^(b3)*dbh))^(b4 + b4ba * standBasalAreaPerHectare + b4bal * basalAreaLarger), psme2022, # a1r, b4r singularity in backsolve, b2r step halving, b2ba, b2bap not significant
                                                        fixedFormula = a1 + a1p + b1 + b1p + b2 + b3 + b4 + b4ba + b4bal ~ 1, randomFormula = b1r ~ 1|stand/plot,
                                                        start = list(fixed = c(a1 = 30, a1p = -10, b1 = 0.1, b1p = 0.1, b2 = -0.03, b3 = -0.05, b4 = 1.1, b4ba = 0.01, b4bal = -0.01)))
...
if (psmeOptions$fitDbhPrimary)
{
  ...
  psmeDiameterFromHeight$ruarkPhysio = fit_gsl_nls("Ruark physio", dbh ~ (a1)*(height - 1.37)^(b1) * exp((b2 + b2ac * cos(pi/180*aspect)) * (height - 1.37)), psme2022, start = list(a1 = 2, b1 = 0.8, b2 = 0.01, b2ac = -0.001)) # { a6, b1, b2 } x { e, s, as, tr, tw }, b2p not significant, a5ac, b1ac also  significant
  ...
  psmeDiameterFromHeight$ruarkRelHtPhysio = fit_gsl_nls("Ruark RelHt physio", dbh ~ (a1)*(height - 1.37)^(b1) * exp((b2 + b2rh * relativeHeight + b2ac * cos(3.14159/180 * aspect)) * (height - 1.37)), psme2022, start = list(a1 = 2, b1 = 0.8, b2 = 0.008, b2ac = -0.001, b2rh = 0.003))
  #psmeDiameterFromHeight$schnute = fit_gsl_nls("Schnute inverse", dbh ~ -1/a1 * log(1 - pmin((1 - exp(-a2))*(height^b1 - 1.37^b1)/(Ha^b1 - 1.3^b1), 0.9999)), psme2022, start = list(a1 = 0.0001, a2 = 0.55, b1 = 1.05, Ha = 90)) # always nan-inf or singular gradient
  ...
  # all GAM forms NaN with Γ link
  psmeDiameterFromHeight$gam = fit_gam("REML GAM", dbh ~ I(height^(1.283 - 0.200 * isPlantation)) + s(height, bs = "ts", by = as.factor(isPlantation), k = 6), data = psme2022, threads = 2) # 10x10 AIC 5324
...
if (psmeOptions$fitDbhMixed)
{
  ...
  psmeDiameterFromHeightMixed$naslund = fit_nlme("Näslund inverse", dbh ~ (a1 + a1r + a1p * isPlantation) * sqrt(height - 1.37) / (1 + (a2 + a2p * isPlantation) * sqrt(height - 1.37)), psme2022, # a2r also tractable
  ...
  psmeDiameterFromHeightMixed$ruarkPhysio = fit_nlme("Ruark physio", dbh ~ (a1)*(height - 1.37)^(b1) * exp((b2 + b2ac * cos(3.14159/180 * aspect) + b2r) * (height - 1.37)), psme2022, # a1r singular precision, b1r also tractable
                                                     fixedFormula = a1 + b1 + b2 + b2ac ~ 1, randomFormula = b2r ~ 1|stand/plot,
                                                     start = list(fixed = c(a1 = 2, b1 = 0.8, b2 = 0.01, b2ac = -0.001)))
  psmeDiameterFromHeightMixed$ruarkRelHt = fit_nlme("Ruark RelHt", dbh ~ (a1)*(height - 1.37)^(b1 + b1r) * exp((b2 + b2rh * relativeHeight) * (height - 1.37)), psme2022, # a1r, b2r also tractable
                                                    fixedFormula = a1 + b1 + b2 + b2rh ~ 1, randomFormula = b1r ~ 1|stand/plot,
                                                    start = list(fixed = c(a1 = 2, b1 = 0.8, b2 = 0.008, b2rh = 0.003)))
  psmeDiameterFromHeightMixed$ruarkRelHtPhysio = fit_nlme("Ruark RelHt physio", dbh ~ (a1)*(height - 1.37)^(b1 + b1r) * exp((b2 + b2rh * relativeHeight + b2ac * cos(3.14159/180 * aspect)) * (height - 1.37)), psme2022, # a1r, b2r also tractable
                                                          fixedFormula = a1 + b1 + b2 + b2ac + b2rh ~ 1, randomFormula = b1r ~ 1|stand/plot,
                                                          start = list(fixed = c(a1 = 2, b1 = 0.8, b2 = 0.008, b2ac = -0.001, b2rh = 0.003)))
  #psmeDiameterFromHeightMixed$schnute = fit_nlme("Schnute inverse", dbh ~ -1/a1 * log(1 - pmin((1 - exp(-a2))*(height^b1 - 1.37^b1)/((Ha + Har)^b1 - 1.3^b1), 0.9999)), psme2022, # fixed form failed to fit
  ...
  psmeDiameterFromHeightMixed$sibbesenReplacePhysio = fit_nlme("Sibbesen replace physio", dbh ~ (a1 + a1p * isPlantation)*(height - 1.37)^((b1)*(height - 1.37)^(b2 + b2p * isPlantation + b2ac * cos(3.14159/180 * aspect) + b2r)), psme2022, # a1r false convergence, b1r also tractable
  ...
  psmeDiameterFromHeightMixed$sibbesenReplaceRelHt = fit_nlme("Sibbesen replace RelHt", dbh ~ a1*(height - 1.37)^((b1)*(height - 1.37)^(b2 + b2p * isPlantation + b2rh * relativeHeight + b2r)), psme2022, # a1r step halving, b1r also tractable
  ...
  psmeDiameterFromHeightMixed$sibbesenReplaceRelHtPhysio = fit_nlme("Sibbesen replace RelHt physio", dbh ~ (a1)*(height - 1.37)^((b1)*(height - 1.37)^(b2 + b2p * isPlantation + b2rh * relativeHeight + b2ac * cos(3.14159/180 * aspect) + b2r)), psme2022, # a1r false convergence, b1r also tractable
...
{
  psmeHeightFromDiameterPreferred = list(gam = fit_gam("REML GAM", height ~ I(dbh^(0.6516 + 0.1398 * isPlantation)) + s(dbh, bs = "ts", by = as.factor(isPlantation), k = 7), data = psme2022, threads = 4, folds = 1, repetitions = 1))
