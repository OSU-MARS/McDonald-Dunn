...
if (psmeOptions$fitDbhPrimary)
{
  ...
  psmeDiameterFromHeight$chapmanReplaceAbat = fit_gsl_nls("Chapman-Richards replace ABA+T", dbh ~ (a1 + a1p * isPlantation)*(exp((b1 + b1aba * bootstrapStandBasalAreaPerHectare)*(height - 1.37)) - 1)^(b2), psme2022, start = list(a1 = 40, a1p = -3, b1 = 0.02, b1aba = -0.0001, b2 = 0.9)) # b1abap not significant, 10x0 RMSE 14.4, AIC 5177
  ...
  psmeDiameterFromHeight$chapmanRichardsAbat = fit_gsl_nls("Chapman-Richards inverse ABA+T", dbh ~ (a1)*log(1 - pmin(((b1 + (b1aba + b1abap * isPlantation) * bootstrapStandBasalAreaPerHectare)*(height - 1.37))^(b2), 0.9999)), psme2022, start = list(a1 = -100, b1 = 0.01, b1aba = -0.00002, b1abap = -0.00001, b2 = 1.0)) # 10x10 RMSE 14.7, AIC 5236
  ...
  psmeDiameterFromHeight$ruarkAbat = fit_gsl_nls("Ruark ABA+T", dbh ~ (a1 + a1aba * bootstrapStandBasalAreaPerHectare)*(height - 1.37)^(b1) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 2.0, a1aba = -0.006, b1 = 0.9, b2 = 0.01, b2p = -0.002)) # a1abap not significant, 10x10 RMSE 14.4, AIC 5296
  #psmeDiameterFromHeight$ruarkAbatB1aba = fit_gsl_nls("Ruark ABA+T", dbh ~ (a1)*(height - 1.37)^(b1 + b1aba * bootstrapStandBasalAreaPerHectare) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 1.8, b1 = 0.8, b1aba = -0.001, b2 = 0.01, b2p = -0.002)) # 10x10 RMSE 14.7, AIC 5318
  #psmeDiameterFromHeight$ruarkAbatB2aba = fit_gsl_nls("Ruark ABA+T", dbh ~ (a1)*(height - 1.37)^(b1) * exp((b2 + b2p * isPlantation + b2aba * bootstrapStandBasalAreaPerHectare) * (height - 1.37)), psme2022, start = list(a1 = 1.9, b1 = 0.8, b2 = 0.02, b2p = -0.001, b2aba = -0.0001)) # 10x10 RMSE 14.6, AIC 5268
  #psmeDiameterFromHeight$ruarkAbatA1bat = fit_gsl_nls("Ruark ABA+T", dbh ~ (a1 + a1bat * basalAreaTaller)*(height - 1.37)^(b1) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 2.0, a1bat = -0.01, b1 = 0.8, b2 = 0.01, b2p = -0.001)) # 10x10 RMSE 14.4, AIC 5188
  #psmeDiameterFromHeight$ruarkAbatB1bat = fit_gsl_nls("Ruark ABA+T", dbh ~ (a1)*(height - 1.37)^(b1 + b1bat * basalAreaTaller) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 1.8, b1 = 0.9, b1bat = -0.001, b2 = 0.01, b2p = -0.001)) # 10x10 RMSE 14.5, AIC 5219
  #psmeDiameterFromHeight$ruarkAbatB2bat = fit_gsl_nls("Ruark ABA+T", dbh ~ (a1)*(height - 1.37)^(b1) * exp((b2 + b2p * isPlantation + b2bat * basalAreaTaller) * (height - 1.37)), psme2022, start = list(a1 = 1.8, b1 = 0.8, b2 = 0.01, b2p = -0.001, b2bat = -0.0001)) # 10x10 RMSE 15.0, AIC 5268
  #psmeDiameterFromHeight$ruarkAbatA1abaBat = fit_gsl_nls("Ruark ABA+T", dbh ~ (a1 + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller)*(height - 1.37)^(b1) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 2.0, a1aba = -0.005, a1bat = -0.004, b1 = 0.9, b2 = 0.01, b2p = -0.002)) # a1abap, a1batp not significant, 10x10 RMSE 15.0, AIC 5304
  ...
  psmeDiameterFromHeight$sibbesenReplaceAbat = fit_gsl_nls("Sibbesen replace ABA+T", dbh ~ (a1)*(height - 1.37)^((b1)*(height - 1.37)^(b2 + b2p * isPlantation + b2aba * bootstrapStandBasalAreaPerHectare)), psme2022, start = list(a1 = 2.0, b1 = 0.7, b2 = 0.1, b2p = -0.007, b2aba = -0.0002)) # a1p, b2abap not significant, 10x10 RMSE 14.6, AIC 5136
  #psmeDiameterFromHeight$sibbesenReplaceAbatA1aba = fit_gsl_nls("Sibbesen replace ABA+T", dbh ~ (a1 + a1aba * bootstrapStandBasalAreaPerHectare)*(height - 1.37)^(b1*(height - 1.37)^(b2 + b2p * isPlantation)), psme2022, start = list(a1 = 2.1, a1aba = -0.006, b1 = 0.7, b2 = 0.1, b2p = -0.005)) # a1p not significant, 10x10 RMSE 14.6, AIC 5241
  #psmeDiameterFromHeight$sibbesenReplaceAbatB1aba = fit_gsl_nls("Sibbesen replace ABA+T", dbh ~ (a1 + a1p * isPlantation)*(height - 1.37)^((b1 + b1aba * bootstrapStandBasalAreaPerHectare)*(height - 1.37)^(b2 + b2p * isPlantation)), psme2022, start = list(a1 = 1.6, a1p = 0.3, b1 = 0.7, b1aba = -0.001, b2 = 0.1, b2p = 0.02)) # 10x10 RMSE 14.9, AIC 5276
  #psmeDiameterFromHeight$sibbesenReplaceAbatA1bat = fit_gsl_nls("Sibbesen replace ABA+T", dbh ~ (a1 + a1bat * basalAreaTaller)*(height - 1.37)^(b1*(height - 1.37)^(b2 + b2p * isPlantation)), psme2022, start = list(a1 = 2.0, a1bat = -0.01, b1 = 0.7, b2 = 0.08, b2p = -0.005)) # a1p not significant, 10x10 RMSE 14.7, AIC 5264
  #psmeDiameterFromHeight$sibbesenReplaceAbatB1bat = fit_gsl_nls("Sibbesen replace ABA+T", dbh ~ (a1)*(height - 1.37)^((b1 + b1bat * basalAreaTaller)*(height - 1.37)^(b2 + b2p * isPlantation)), psme2022, start = list(a1 = 2.0, b1 = 0.8, b1bat = -0.001, b2 = 0.07, b2p = 0.006)) # a1p not significant, 10x10 RMSE 14.8, AIC 5180
  ...
  #psmeDiameterFromHeight$randomForestAbatRelHt = fit_ranger("random forest RelHt ABA+T", c("dbh", "height", "bootstrapStandBasalAreaPerHectare", "relativeHeight"), # from setup.R VSURF
...
if (psmeOptions$fitDbhMixed)
{
  ...
  #psmeDiameterFromHeightMixed$chapmanReplaceAbat = fit_nlme("Chapman-Richards replace ABA+T", dbh ~ (a1 + a1r + a1p * isPlantation)*(exp((b1 + b1aba * bootstrapStandBasalAreaPerHectare)*(height - 1.37)) - 1)^(b2), psme2022, # a1r max iterations, b1r, b2r singularity in backsolve
  #                                                          fixedFormula = a1 + a1p + b1 + b1aba + b2 ~ 1, randomFormula = a1r ~ 1|stand/plot,
  #                                                          start = list(fixed = c(a1 = 40, a1p = -3, b1 = 0.02, b1aba = -0.0001, b2 = 0.9)))
...
if (htDiaOptions$includeInvestigatory)
{
  ...
  psmeDbhGam = gam(dbh ~ I((0.668 + 0.607 * isPlantation) * height^(1.283 - 0.200 * isPlantation)) + 
                         ...
                         s(basalAreaTaller, bs = "ts", k = 6) + # plantation not significant, linear-ish overally, arguably piecewise linear with breakpoint ~15 m²/ha
