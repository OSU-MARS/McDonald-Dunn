...
if (psmeOptions$fitHeightPrimary)
{
  ...
  psmeHeightFromDiameter$chapmanRichardsBalRelDbh = fit_gsl_nls("Chapman-Richards BA+L RelDbh", height ~ 1.37 + (a1 + a1rd * relativeDiameter) * (1 - exp((b1)*dbh))^(b2 + b2bal * basalAreaLarger), psme2022, start = list(a1 = 70, a1rd = -1.7, b1 = -0.015, b2 = 1.3, b2bal = -0.005)) # a1ba, a1rdp, b1rdp, b2rdp not significant, a1rd, b1rd also significant
  #psmeHeightFromDiameter$chapmanRichardsBalRelDbhB2 = fit_gsl_nls("Chapman-Richards BA+L RelDbh", height ~ 1.37 + (a1 + a1ba * standBasalAreaPerHectare) * (1 - exp((b1)*dbh))^(b2 + b2rd * relativeDiameter + b2bal * basalAreaLarger), psme2022, start = list(a1 = 57, a1ba = 0.05, b1 = -0.02, b2 = 1.25, b2rd = 0.1, b2bal = -0.005))
  ...
  psmeHeightFromDiameter$chapmanRichardsBalPhysio = fit_gsl_nls("Chapman-Richards BA+L physio", height ~ 1.37 + (a1 + a1ba * standBasalAreaPerHectare) * (1 - exp((b1 + b1ac * cos(3.14159/180 * aspect))*dbh))^(b2 + b2p * isPlantation + b2bal * basalAreaLarger), psme2022, start = list(a1 = 55, a1ba = 0.1, b1 = -0.01, b1ac = -0.001, b2 = 1.3, b2p = 0.1, b2bal = -0.008))
  ...
  # a1ac, b1ac, b2ac, b3ac, b4ac forms all quite close with preference to a1ac over b1ac only on AIC parsimony
  ...
  psmeHeightFromDiameter$sharmaPartonBalPhysioRelDbh = fit_gsl_nls("Sharma-Parton BA+L RelDbh physio", height ~ 1.37 + (a1 + a1rd * relativeDiameter)*topHeight^(b1 + b1as * sin(3.14159/180 * aspect) + b1ac * cos(3.14159/180 * aspect)) * (1 - exp((b2)*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^(b3)*dbh))^(b4), psme2022, start = list(a1 = 25, a1rd = 0.4, b1 = 0.3, b1as = -0.04, b1ac = 0.01, b2 = -0.02, b3 = -0.07, b4 = 1.0))
  psmeHeightFromDiameter$sharmaPartonBalRelDbh = fit_gsl_nls("Sharma-Parton BA+L RelDbh", height ~ 1.37 + (a1 + a1rd * relativeDiameter)*topHeight^(b1) * (1 - exp(b2*(standTreesPerHectare/(standBasalAreaPerHectare + basalAreaLarger))^b3 * dbh))^b4, psme2022, start = list(a1 = 22, a1rd = 0.4, b1 = 0.3, b2 = -0.02, b3 = -0.04, b4 = 1.0)) # a1rdp, b1rdp, b2rd, b2rdp, b3rd, b3rdp, b4rdp not significant, b1rd, b4rd not reliably significant
  ...
  psmeHeightFromDiameter$sharmaZhangBal = fit_gsl_nls("Sharma-Zhang BA+L", height ~ 1.37 + (a1 + a1p * isPlantation + a1ba * standBasalAreaPerHectare + a1bal * basalAreaLarger)*standBasalAreaPerHectare^(b1 + b1p * isPlantation) * (1 - exp((b2)*standTreesPerHectare^b3 * dbh))^(b4), psme2022, start = list(a1 = 50, a1p = -10, a1ba = -0.1, a1bal = 0.2, b1 = 0.1, b1p = 0.1, b2 = -0.03, b3 = -0.02, b4 = 1.1)) # a1bap, a1balp, { b1, b3 } x { bal, balp }, b1ba, b3, b4bap, b4balp not significant, b4ba, b4bal also significant, 10x10 AIC 3779
  ...
  # a1bal and b2bal forms MAE and AIC interchangeable
  psmeHeightFromDiameter$weibullBal = fit_gsl_nls("Weibull BA+L", height ~ 1.37 + (a1) * (1 - exp(b1*dbh^(b2 + b2bal * basalAreaLarger))), psme2022, start = list(a1 = 60, b1 = -0.01, b2 = 1.0, b2bal = 0.002)) # a1balp, b1ba, b1bap, b1bal, b2ba, b2balp, b2bal, b2balp not significant, a1bal also significant
...
if (psmeOptions$fitHeightMixed)
{
  ...
  psmeHeightFromDiameterMixed$chapmanRichardsBalRelDbh = fit_nlme("Chapman-Richards BA+L RelDbh", height ~ 1.37 + (a1 + a1r + a1rd * relativeDiameter) * (1 - exp(b1*dbh))^(b2 + b2bal * basalAreaLarger), psme2022,
...
if (psmeOptions$fitDbhPrimary)
{
  ...
  #psmeDiameterFromHeight$chapmanReplaceAbat = fit_gsl_nls("Chapman-Richards replace ABA+T", dbh ~ (a1 + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller)*(exp((b1 + b1p * isPlantation)*(height - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2022, start = list(a1 = 75, a1p = -35, a1aba = -0.07, a1bat = 0.5, b1 = 0.018, b1p = 0.01, b2 = 0.7, b2p = 0.07))
  ...
  #psmeDiameterFromHeight$chapmanRichardsAbat = fit_gsl_nls("Chapman-Richards inverse ABA+T", dbh ~ (a1 + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller)*log(1 - pmin(((b1 + b1p * isPlantation)*(height - 1.37))^b2, 0.9999)), psme2022, start = list(a1 = -175, a1p = 100, a1aba = 0.14, a1bat = 1.2, b1 = 0.007, b1p = 0.0057, b2 = 0.79))
  ...
  #psmeDiameterFromHeight$powerAbat = fit_gsl_nls("power ABA+T", dbh ~ (a1 + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + (a1bat + a1batp * isPlantation) * basalAreaTaller)*(height - 1.37)^(b1 + b1p*isPlantation), psme2022, start = list(a1 = 2.14, a1p = -0.051, a1aba = 0.00085, a1bat = -0.0065, a1batp = -0.0038, b1 = 0.963, b1p = -0.053))
  ...
  #psmeDiameterFromHeight$ruarkAbat = fit_gsl_nls("Ruark ABA+T", dbh ~ (a1 + a1bat * basalAreaTaller)*(height - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 2.5, a1bat = -0.03, b1 = 0.92, b1p = -0.2, b2 = 0, b2p = 0.013))
  #psmeDiameterFromHeight$ruarkAbatPhysio = fit_gsl_nls("Ruark ABA+T physio", dbh ~ (a1 + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1ac * cos(3.14159/180 * aspect))*(height - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 2.5, a1aba = -0.004, a1bat = -0.017, a1ac = -0.05, b1 = 0.93, b1p = -0.19, b2 = 0.002, b2p = 0.012))
  #psmeDiameterFromHeight$ruarkAbatRelHt = fit_gsl_nls("Ruark ABA+T RelHt", dbh ~ (a1 + a1bat * basalAreaTaller + a1rh * relativeHeight)*(height - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 2.5, a1bat = -0.02, a1rh = 0.2, b1 = 0.91, b1p = -0.16, b2 = 0, b2p = 0.009))
  #psmeDiameterFromHeight$ruarkAbatRelHtPhysio = fit_gsl_nls("Ruark ABA+T RelHt physio", dbh ~ (a1 + a1bat * basalAreaTaller + a1aba * bootstrapStandBasalAreaPerHectare + a1ac * cos(3.14159/180 * aspect) + a1rh*relativeHeight)*(height - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, start = list(a1 = 2.4, a1bat = -0.013, a1aba = -0.0037, a1ac = -0.05, a1rh = 0.1, b1 = 0.94, b1p = -0.16, b2 = 0.001, b2p = 0.01))
  ...
  psmeDiameterFromHeight$ruarkRelHt = fit_gsl_nls("Ruark RelHt", dbh ~ (a1)*(height - 1.37)^(b1) * exp((b2 + b2rh * relativeHeight) * (height - 1.37)), psme2022, start = list(a1 = 2, b1 = 0.8, b2 = 0.008, b2rh = 0.003)) # a1rh, b1rh also significant, b2rh-b4p not mutually significant
  ...
  #psmeDiameterFromHeight$sibbesenReplaceAbat = fit_gsl_nls("Sibbesen replace ABA+T", dbh ~ (a1 + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller)*(height - 1.37)^(b1*(height - 1.37)^(b2 + b2p * isPlantation)), psme2022, start = list(a1 = 4.4, a1p = -1.6, a1aba = -0.006, a1bat = -0.03, b1 = 0.61, b2 = 0.071, b2p = 0.021))
  #psmeDiameterFromHeight$sibbesenReplaceAbatPhysio = fit_gsl_nls("Sibbesen replace ABA+T physio", dbh ~ (a1 + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1ac * cos(3.14159/180 * aspect))*(height - 1.37)^(b1*(height - 1.37)^b2), psme2022, start = list(a1 = 3.2, a1p = -0.6, a1aba = -0.016, a1bat = -0.016, a1ac = -0.057, b1 = 0.66, b2 = 0.077))
  #psmeDiameterFromHeight$sibbesenReplaceAbatRelHt = fit_gsl_nls("Sibbesen replace ABA+T RelHt", dbh ~ (a1 + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1rh * relativeHeight)*(height - 1.37)^(b1*(height - 1.37)^(b2 + b2p * isPlantation)), psme2022, start = list(a1 = 3.9, a1p = -1.0, a1aba = -0.006, a1bat = -0.01, a1rh = 0.15, b1 = 0.61, b2 = 0.071, b2p = 0.021))
  #psmeDiameterFromHeight$sibbesenReplaceAbatRelHtPhysio = fit_gsl_nls("Sibbesen replace ABA+T RelHt physio", dbh ~ (a1 + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1ac * cos(3.14159/180 * aspect) + a1rh * relativeHeight)*(height - 1.37)^(b1*(height - 1.37)^b2), psme2022, start = list(a1 = 3.1, a1p = -0.5, a1aba = -0.005, a1bat = -0.01, a1ac = -0.06, a1rh = 0.1, b1 = 0.65, b2 = 0.07))
  ...
  psmeDiameterFromHeight$sibbesenReplaceRelHt = fit_gsl_nls("Sibbesen replace RelHt", dbh ~ (a1)*(height - 1.37)^((b1)*(height - 1.37)^(b2 + b2p * isPlantation + b2rh * relativeHeight)), psme2022, start = list(a1 = 2, b1 = 0.8, b2 = 0.05, b2p = -0.002, b2rh = 0.01)) # a1p not significant, a1rh, b1rh also significant
...
if (psmeOptions$fitDbhMixed)
{
  ...
  #psmeDiameterFromHeightMixed$chapmanReplaceAbat = fit_nlme("Chapman-Richards replace ABA+T", dbh ~ (a1 + a1r + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller)*(exp((b1 + b1p * isPlantation)*(height - 1.37)) - 1)^(b2 + b2p * isPlantation), psme2022, 
  ...
  #psmeDiameterFromHeightMixed$chapmanRichardsAbat = fit_nlme("Chapman-Richards inverse ABA+T", dbh ~ (a1 + a1r + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller)*log(1 - pmin(((b1 + b1p * isPlantation)*(height - 1.37))^b2, 0.9999)), psme2022, 
  ...
  #                                               fixedFormula = a1 + a1p + b1 + b1p ~ 1, randomFormula = a1r ~ 1|stand/plot,
  #                                               start = list(fixed = c(a1 = 0.7, a1p = 0.4, b1 = 1.2, b1p = -0.2)))
  #psmeDiameterFromHeightMixed$powerAbat = fit_nlme("power ABA+T", dbh ~ (a1 + a1r + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + (a1bat + a1batp * isPlantation) * basalAreaTaller)*(height - 1.37)^(b1 + b1p*isPlantation), psme2022, 
  #                                                 fixedFormula = a1 + a1p + a1aba + a1bat + b1 + b1p ~ 1, randomFormula = a1r ~ 1|stand/plot,
  ...
  #                                               start = list(fixed = c(a1 = 2, b1 = 0.8, b2 = 0.01)))
  #psmeDiameterFromHeightMixed$ruarkAbat = fit_nlme("Ruark ABA+T", dbh ~ (a1 + a1r + a1bat * basalAreaTaller)*(height - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, 
  ...
  #psmeDiameterFromHeightMixed$ruarkAbatPhysio = fit_nlme("Ruark ABA+T physio", dbh ~ (a1 + a1r + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1ac * cos(3.14159/180 * aspect))*(height - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022,
  ...
  #psmeDiameterFromHeightMixed$ruarkAbatRelHt = fit_nlme("Ruark ABA+T RelHt", dbh ~ (a1 + a1r + a1bat * basalAreaTaller + a1rh * relativeHeight)*(height - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022, 
  ...
  #psmeDiameterFromHeightMixed$ruarkAbatRelHtPhysio = fit_nlme("Ruark ABA+T RelHt physio", dbh ~ (a1 + a1r + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1ac * cos(3.14159/180 * aspect) + a1rh*relativeHeight)*(height - 1.37)^(b1 + b1p * isPlantation) * exp((b2 + b2p * isPlantation) * (height - 1.37)), psme2022,
  ...
  #                                                         start = list(fixed = c(a1 = 1.6, a1p = 0.4, b1 = 0.7, b2 = 0.1, b2p = -0.02)))
  #psmeDiameterFromHeightMixed$sibbesenReplaceAbat = fit_nlme("Sibbesen replace ABA+T", dbh ~ (a1 + a1r + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller)*(height - 1.37)^(b1*(height - 1.37)^(b2 + b2p * isPlantation)), psme2022, 
  ...
  #psmeDiameterFromHeightMixed$sibbesenReplaceAbatPhysio = fit_nlme("Sibbesen replace ABA+T physio", dbh ~ (a1 + a1r + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1ac * cos(3.14159/180 * aspect))*(height - 1.37)^(b1*(height - 1.37)^b2), psme2022,
  ...
  #psmeDiameterFromHeightMixed$sibbesenReplaceAbatRelHt = fit_nlme("Sibbesen replace ABA+T RelHt", dbh ~ (a1 + a1r + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1rh * relativeHeight)*(height - 1.37)^(b1*(height - 1.37)^(b2 + b2p * isPlantation)), psme2022, 
  ...
  #psmeDiameterFromHeightMixed$sibbesenReplaceAbatRelHtPhysio = fit_nlme("Sibbesen replace ABA+T RelHt physio", dbh ~ (a1 + a1r + a1p * isPlantation + a1aba * bootstrapStandBasalAreaPerHectare + a1bat * basalAreaTaller + a1ac * cos(3.14159/180 * aspect) + a1rh * relativeHeight)*(height - 1.37)^(b1*(height - 1.37)^b2), psme2022,
