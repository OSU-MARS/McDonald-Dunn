...
(modelCounts = heightDiameterFitStats %>% filter(fitSet == "primary") %>% 
  group_by(responseVariable, species) %>%
...
heightDiameterFitStats %>% group_by(responseVariable, species) %>% # plantation effect quantiles
...
# only by chance.
print(left_join(left_join(left_join(left_join(preferredForms %>% group_by(responseVariable, species) %>% slice_max(aucMab, n = 1, with_ties = FALSE) %>% select(responseVariable, species, mabName, mabFitting),
                                              preferredForms %>% group_by(responseVariable, species) %>% slice_max(aucMae, n = 1, with_ties = FALSE) %>% select(responseVariable, species, maeName, maeFitting),
                                              by = join_by(responseVariable, species)),
                                    preferredForms %>% group_by(responseVariable, species) %>% slice_max(aucRmse, n = 1, with_ties = FALSE) %>% select(responseVariable, species, rmseName, rmseFitting),
                                    by = join_by(responseVariable, species)),
                          preferredForms %>% group_by(responseVariable, species) %>% slice_max(aucDeltaAicN, n = 1, with_ties = FALSE) %>% select(responseVariable, species, aicName, aicFitting),
                          by = join_by(responseVariable, species)),
                preferredForms %>% group_by(responseVariable, species) %>% slice_max(aucNse, n = 1, with_ties = FALSE) %>% select(responseVariable, species, nseName, nseFitting),
                by = join_by(responseVariable, species)) %>%
        pivot_longer(cols = ends_with("Name"), names_to = "statistic", names_pattern = "(.*)Name", values_to = "name") %>%
        mutate(fitting = case_match(statistic, "mab" ~ mabFitting, "mae" ~ maeFitting, "rmse" ~ rmseFitting, "aic" ~ aicFitting, "nse" ~ nseFitting)) %>%
        select(-mabFitting, -maeFitting, -rmseFitting, -aicFitting, -nseFitting) %>%
...
ggplot() +
  geom_violin(aes(x = deltaMapb, y = species, color = responseVariable), deltaMapb %>% filter(responseVariable == "height"), quantiles = c(0.25, 0.5, 0.75), quantile.linetype = "solid") +
  labs(x = "generalization ΔMAB, %", y = NULL, color = NULL, title = "a) height") +
ggplot() +
  geom_violin(aes(x = deltaMapb, y = species, color = responseVariable), deltaMapb %>% filter(responseVariable == "DBH"), quantiles = c(0.25, 0.5, 0.75), quantile.linetype = "solid") +
...
ggplot(heightDiameterFitStats %>% filter(responseVariable == "height", isBaseForm)) +
...
ggplot(heightDiameterFitStats %>% filter(responseVariable == "DBH", isBaseForm)) +
  geom_histogram(aes(x = if_else(nse > -0.05, nse, NA_real_), y = 100 * after_stat(count / sum(count)), fill = species), binwidth = 0.025, na.rm = TRUE) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 24)) +
  ...
  scale_x_continuous(breaks = seq(0, 1, by = 0.2)) +
ggplot(heightDiameterFitStats %>% filter(responseVariable == "height", isBaseForm == FALSE)) +
  geom_histogram(aes(x = if_else(nse > -0.05, nse, NA_real_), y = 100 * after_stat(count / sum(count)), fill = species), binwidth = 0.025, na.rm = TRUE) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 24)) +
  ...
  scale_x_continuous(breaks = seq(0, 1, by = 0.2)) +
ggplot(heightDiameterFitStats %>% filter(responseVariable == "DBH", isBaseForm == FALSE)) +
  geom_histogram(aes(x = if_else(nse > -0.05, nse, NA_real_), y = 100 * after_stat(count / sum(count)), fill = species), binwidth = 0.025, na.rm = TRUE) +
  coord_cartesian(xlim = c(0, 1), ylim = c(0, 24)) +
...
#ggsave("trees/height-diameter/figures/Figure 06 PSME-ACMA3 curves.png", height = 12, width = 20, units = "cm", dpi = figureDpi)
#ggsave("trees/height-diameter/figures/Figure 06 PSME-ACMA3 curves.tif", height = 12, width = 20, units = "cm", dpi = figureDpi, compression = "lzw+p")
#ggsave("trees/height-diameter/figures/Figure 06 PSME-ACMA3 curves 1000.pdf", height = 12, width = 20, units = "cm", dpi = figureDpi, device = cairo_pdf, fallback_resolution = figureDpi)


## Figure 7: grand fir and other species preferred models
#print(preferredForms %>% filter(speciesModel %in% c("grand fir", "other")) %>% select(-mabName, -aucMab, -nseName, -aucNse) %>% rename(respVar = responseVariable, base = isBaseForm, aucAic = aucDeltaAicN) %>% mutate(species = factor(species, labels = c("PSME", "ACMA3", "ABGR", "other"), levels = c("Douglas-fir", "bigleaf maple", "grand fir", "other species")), maeName = str_trunc(maeName, 28, ellipsis = ""), rmseName = str_trunc(rmseName, 28, ellipsis = ""), aicName = str_trunc(aicName, 28, ellipsis = "")), n = 32)
...
#writexl::write_xlsx(list(preferredModelStats = selectedModels %>% select(species, responseVariable, name, mab, mapb, mae, mape, rmse, rmpse, deltaAicN, nse), "trees/height-diameter/figures/selected model stats.xlsx")
...
accuracyCorrelation = bind_rows(as.data.frame(cor(heightDiameterFitStats %>% filter(responseVariable == "height") %>% 
                                  ...
                                  mutate(responseVariable = "height"),
                                as.data.frame(cor(heightDiameterFitStats %>% filter(responseVariable == "DBH") %>% 
                                                    select(mapb, mape, rmse, deltaAicN, nse), use = "pairwise.complete.obs")) %>%
                                  tibble::rownames_to_column("metricX") %>% gather("metricY", "correlation", -metricX) %>%
                                  mutate(responseVariable = "DBH")) %>%
...
heightFromDiameterAccuracyLevels = heightDiameterFitStats %>% 
  ...
  summarize(weightedMae = sum(speciesFraction * meanPenalizedMae),
            .groups = "drop") %>%
  arrange(weightedMae)
heightFromDiameterResults = heightDiameterFitStats %>% 
...
diameterFromHeightAccuracyLevels = heightDiameterFitStats %>% 
...
diameterFromHeightResults = heightDiameterFitStats %>% 
