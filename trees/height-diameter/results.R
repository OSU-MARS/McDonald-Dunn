...
figureDpi = 300
mixedModelFittings = c("lme", "nlme", "gamm")
speciesGroupColors = c("forestgreen", "blue2", "green3", "grey65") # Douglas-fir, grand fir, bigleaf maple, other species

check_cross_validation = function(name, data, folds, repetitions, suffixIfRandomCrossValidation)
{
  if ((max(data$fold, na.rm = TRUE) != folds) | (max(data$repetition, na.rm = TRUE) != repetitions))
  {
    stop(paste0(name, " data mismatch: ", folds, "x", repetitions, suffixIfRandomCrossValidation), " has ", max(data$folds), "x", max(data$repetitions), " cross validation.")
  }
}

get_mismatched_cross_validations = function(data, folds, repetitions)
{
  return(data %>% mutate(expectedValidations = folds * repetitions) %>% group_by(fitSet, fitting, responseVariable, species, name) %>% 
           summarize(expectedValidations = expectedValidations[1], actualValidations = n(), distinct = any(as.logical(distinct)), .groups = "drop") %>% # any() warns if not passed a logical
           filter(actualValidations != expectedValidations))
}
...
{
  if (length(unique(heightDiameterFitStats$crossValidation)) != 1)
  {
    stop("Fit statistics encompass multiple cross validation methods. This is not currently supported.")
  }
    ...
    heightDiameterModelAucs = heightDiameterFitStats %>%
        ...
        #purrr::map_dfr(function(fitResults) # for debugging
      {
        ...
        pairwiseAucs = bind_rows(purrr::map2(matchingModels$name, matchingModels$fitting, function(otherModelName, otherModelFitting)
        {
          ...
          {
            stop(paste0(fitResults$species[1], " ", fitResults$responseVariable[1], " ", fitResults$name[1], " ", fitResults$fitting[1], " @", nrow(fitResults), " folds x ", otherModelName, " ", otherModelFitting, " @ ", nrow(otherFitResults), " folds: number of cross validation folds mismatched between fits."), quote = FALSE)
          }
          ...
          #outOfBoundsGuess = c(fitResults$pctOutOfRange, otherFitResults$pctOutOfRange)
      ...
      mutate(folds = max(heightDiameterFitStats$fold, na.rm = TRUE),
             ...
             crossValidation = heightDiameterFitStats$crossValidation[1])
    })
  Sys.time() - aucStart
...
get_model_comparison = function(heightDiameterModelRanking, predictedTreeAttribute = "height", heightDiameterModelRankingForDisplaySort = heightDiameterModelRanking)
{
  ...
  return(heightDiameterModelRanking %>% filter(responseVariable == predictedTreeAttribute) %>%
           mutate(isMixed = fitting %in% mixedModelFittings,
...
get_model_ranking = function(heightDiameterModelAucs)
{
  ...
  heightDiameterModelRanking = heightDiameterModelAucs %>%
    group_by(responseVariable, species, name, fitting) %>%
    summarize(folds = folds[1], repetitions = repetitions[1], crossValidation = crossValidation[1],
    ...
    mutate(aucBlended = if_else(distinct, 0.3 * aucBounds + 
                                          0.3 * if_else(responseVariable == "height", aucMae, aucRmse) + 
                                          0.1 * if_else(responseVariable == "DBH", aucMae, aucRmse) + 
                                          0.3 * if_else(fitting == "ranger", 0.5, aucDeltaAicN), # +
                                          #0.1 * aucNse, 
                                0.5),
...
# summarize predictor variable selection by inference from regression coefficients
# Deprecated in favor of read_predictor_results().
...
read_fit_stats = function(crossValidation, folds, repetitions)
{
  if (crossValidation == "randomByCruiseRecord") 
  { 
    dataSubdirectory = "blocked by cruise record/"
    suffixIfRandomCrossValidation = " random" 
  } else { 
    dataSubdirectory = "blocked by stand/"
    suffixIfRandomCrossValidation = "" 
  }
  
  psmeFitStats = readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "PSME fit stats ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))
  check_cross_validation("Douglas-fir", psmeFitStats, folds, repetitions, suffixIfRandomCrossValidation)
  abgrFitStats = readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "ABGR fit stats ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))
  check_cross_validation("grand fir", abgrFitStats, folds, repetitions, suffixIfRandomCrossValidation)
  acmaFitStats = readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "ACMA3 fit stats ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))
  check_cross_validation("bigleaf maple", acmaFitStats, folds, repetitions, suffixIfRandomCrossValidation)
  otherFitStats = readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "other fit stats ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))
  check_cross_validation("other species", otherFitStats, folds, repetitions, suffixIfRandomCrossValidation)
  
  heightDiameterFitStats = bind_rows(psmeFitStats, abgrFitStats, acmaFitStats, otherFitStats) %>%
    mutate(crossValidation = crossValidation,
           baseName = if_else(word(name) %in% c("REML", "modified", "unified"), paste(word(name, 1), word(name, 2)), word(name)),
    ...
    mutate(#pctOutOfRange = 100 * (nPredictionNA + nPredictionNegative) / n, # or nPredictionBelowMin instead of nPredictionNegative
           pctOutOfRange = 100 * (nPredictionNA + nPredictionNegative + nPredictionAboveMax) / n,
    ...
    ungroup() %>%
    relocate(fitSet, responseVariable, species, name, fitting, crossValidation, fold, repetition)
  
  # check for duplicate naming in model fits or 
  mismatchedCrossValidations = get_mismatched_cross_validations(heightDiameterFitStats, folds, repetitions)
  presumedFitFailures = sum(mismatchedCrossValidations$actualValidations == 1)
  if (nrow(mismatchedCrossValidations) != presumedFitFailures)
  {
    print(mismatchedCrossValidations, n = nrow(mismatchedCrossValidations))
    stop(paste0("Found unexpected ", folds, "x", repetitions, " cross validation counts when reading fit statistics."))
  }

  return(heightDiameterFitStats)
}

read_model_coefficients = function(crossValidation, folds, repetitions, heightDiameterFitStats)
{
  if (crossValidation == "randomByCruiseRecord") 
  { 
    dataSubdirectory = "blocked by cruise record/"
  ...
  psmeCoefficients = readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "PSME coefficients ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))
  check_cross_validation("Douglas-fir", psmeCoefficients, folds, repetitions, suffixIfRandomCrossValidation)
  abgrCoefficients = readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "ABGR coefficients ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))
  check_cross_validation("grand fir", abgrCoefficients, folds, repetitions, suffixIfRandomCrossValidation)
  acmaCoefficients = readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "ACMA3 coefficients ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))
  check_cross_validation("bigleaf maple", acmaCoefficients, folds, repetitions, suffixIfRandomCrossValidation)
  otherCoefficients = readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "other coefficients ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))
  check_cross_validation("other species", otherCoefficients, folds, repetitions, suffixIfRandomCrossValidation)
  
  # generalizing DBH predictors are all in any_of() to accommodate with only height results
  heightDiameterCoefficients = left_join(bind_rows(psmeCoefficients, abgrCoefficients, acmaCoefficients, otherCoefficients) %>%
    ...
    mutate(crossValidation = crossValidation,
           isConverged = as.logical(isConverged)) %>%
    select(-fixedEffects) %>%
    relocate(responseVariable, species, fitSet, name, fitting, distinct, isBaseForm, hasRelative, hasStand, hasPhysio, crossValidation, repetition, fold, n, fitTimeInS, isConverged, pctOutOfRange, mae, mape, rmse, rmspe, deltaAicN, nse, meanAbsolutePlantationEffect,	meanAbsolutePercentPlantationEffect, 
             a0, a1, a1p, a1rd, any_of(c("a1rh", "a1bal", "a1balr", "a1aba", "a1aat", "a1e", "a1s", "a1as", "a1ac", "a1tr", "a1tw")), a2, a2rd, a2bal, a2balr, a2p, a3, a3p, b1, b1p, b1rd, b1ba, b1bal, b1balr, any_of(c("b1aba", "b1abap", "b1aat", "b1aatp", "b1rh")), b1as, b1ac, any_of(c("b1e", "b1tw")), b2, b2p, b2rd, b2rdp, any_of(c("b2rh")), b2ba, b2bal, b2balr, any_of(c("b2aba", "b2aat", "b2s", "b2as", "b2ac", "b2e", "b2tw")), b3, b3rd, b3bal, b3balr, any_of(c("b3tr")), b4, b4p, b4d, any_of(c("b4rd")), b4ba, b4bal, b4balr, b4e, b4as, b4ac, any_of(c("b4tw")), Ha, Hap, d, any_of(c("dp")), kU, randomEffect)
  
  mismatchedCrossValidations = get_mismatched_cross_validations(heightDiameterCoefficients, folds, repetitions)
  presumedFitFailures = sum(mismatchedCrossValidations$actualValidations == 1)
  if (nrow(mismatchedCrossValidations) != presumedFitFailures)
  {
    print(mismatchedCrossValidations, n = nrow(mismatchedCrossValidations))
...
read_predictor_results = function(crossValidation, folds, repetitions)
{
  if (crossValidation == "randomByCruiseRecord") 
  { 
    dataSubdirectory = "blocked by cruise record/"
  ...
  # no need to check cross validation as predictors are reported per model
  predictorVariables = bind_rows(readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "PSME predictors ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds")),
                                 readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "ABGR predictors ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds")),
                                 readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "ACMA3 predictors ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds")),
                                 readRDS(paste0("trees/height-diameter/data/", dataSubdirectory, "other predictors ", folds, "x", repetitions, suffixIfRandomCrossValidation, ".Rds"))) %>% 
    filter(fitSet == "primary") %>%
    mutate(species = factor(species, labels = c("Douglas-fir", "grand fir", "bigleaf maple", "other species"), levels = c("PSME", "ABGR", "ACMA3", "other")),
           across(where(is.logical), ~replace_na(.x, FALSE)), # while NAs are filled at the species level, binding rows introduces additional NAs from the union of columns
           distinct = replace_na(distinct, TRUE), # distinct = NA indicates failure of a fit which is distinct
           nBasalArea = standBasalAreaPerHectare + basalAreaLarger + if(exists("bootstrapStandBasalAreaPerHectare")) { bootstrapStandBasalAreaPerHectare } else { 0 } + if(exists("basalAreaTaller")) { basalAreaTaller } else { 0 },
           nPhysio = elevation + slope + aspect + terrainRoughness + topographicWetnessFD8f,
           nRelative = relativeDiameter | if(exists("relativeHeight")) { relativeHeight } else { 0 })
...
#   summarize(n = n(),
#             hasBasalArea = sum(standBasalAreaPerHectare | basalAreaLarger | bootstrapStandBasalAreaPerHectare | basalAreaTaller),
#             hasPhysio = sum(elevation | slope | aspect | topographicWetnessFD8f | terrainRoughness),
#             hasPlantation = sum(isPlantation),
#             hasRelDbhOrRelHt = sum(relativeHeight | relativeDiameter),
#             nBasalAreaDistinct = sum(distinct * (standBasalAreaPerHectare | basalAreaLarger | bootstrapStandBasalAreaPerHectare | basalAreaTaller)),
#             nPhysioDistinct = sum(distinct * (elevation | slope | aspect | topographicWetnessFD8f | terrainRoughness)),
#             nDistinctRelHtOrDbh = sum(distinct * (relativeHeight | relativeDiameter)),
#             nDistinctBAorAba = sum(distinct * (standBasalAreaPerHectare | bootstrapStandBasalAreaPerHectare)),
#             nDistinctBalOrAat = sum(distinct * (basalAreaLarger | basalAreaTaller)),
#             nDistinctElevation = sum(distinct * elevation),
#             nDistinctSlope = sum(distinct * slope),
#             nDistinctAspect = sum(distinct * aspect),
#             nDistinctTerrainRoughness = sum(distinct * terrainRoughness),
#             nDistinctTopographicWetness = sum(distinct * topographicWetnessFD8f),
...
#heightDiameterFitStats10x10 = read_fit_stats("blockedByStand", folds = 10, repetitions = 10)
#heightDiameterCoefficients10x10 = read_model_coefficients(heightDiameterFitStats10x10, "blockedByStand", folds = 10, repetitions = 10)
#heightDiameterFitStats10x25 = read_fit_stats("blockedByStand", folds = 10, repetitions = 25)
#heightDiameterCoefficients10x25 = read_model_coefficients(heightDiameterFitStats10x25, "blockedByStand", folds = 10, repetitions = 25)
...
(modelCounts = heightDiameterFitStats10x100 %>% filter(fitSet == "primary") %>% 
  group_by(responseVariable, species) %>%
...
(actualAucs = heightDiameterModelAucs10x100 %>% filter(name != otherModelName) %>%
    group_by(responseVariable, species) %>% 
...
heightDiameterFitStats10x100 %>% filter(distinct) %>% 
...
get_top_base_form = function(preferredForms, statisticAucSuffix)
{
  ...
  return(preferredForms %>% filter(isBaseForm) %>% group_by(responseVariable, species) %>% slice_max(.data[[str_c("auc", statisticAucSuffix)]], n = 1) %>%
           mutate(nTiedForms = n(), isMixed = maeFitting %in% c("lme", "nlme", "gamm")) %>% # first try to break ties by excluding more complex mixed models in favor of fixed effect models
           filter((nTiedForms == 1) | (isMixed == FALSE)) %>%
           slice_tail(n = 1) %>% # but ties may still exist, in particular between Hossfeld IV and Michaelis-Menten as they're identical for certain coefficient values 
...
{
  return(left_join(left_join(left_join(get_top_base_form(preferredForms, "Mae"), 
                                       get_top_base_form(preferredForms, "Rmse"), 
                                       by = join_by(responseVariable, species, folds, repetitions)),
                             get_top_base_form(preferredForms, "DeltaAicN"),
                             by = join_by(responseVariable, species, folds, repetitions)),
                   get_top_base_form(preferredForms, "Nse"), 
                   by = join_by(responseVariable, species, folds, repetitions)) %>%
...
heightDiameterFitStats10x100 %>% group_by(responseVariable, species) %>% # plantation effect quantiles
...
predictorVariableStats %>%
  mutate(balAat = nDistinctBalOrAat / nBasalAreaDistinct, baAba = nDistinctBAorAba / nBasalAreaDistinct,
         elev = nDistinctElevation / nDistinctPhysio, slope = nDistinctSlope / nDistinctPhysio, sinAspect = nDistinctSinAspect / nDistinctPhysio, cosAspect = nDistinctCosAspect / nDistinctPhysio, roughness = nDistinctTerrainRoughness / nDistinctPhysio, wetness = nDistinctTopographicWetness / nDistinctPhysio) %>%
...
print(left_join(left_join(left_join(heightDiameterModelsPreferred10x100 %>% group_by(responseVariable, species) %>% slice_max(aucMae, n = 1, with_ties = FALSE) %>% select(responseVariable, species, maeName, maeFitting),
                                    ...
                                    by = join_by(responseVariable, species)),
                          heightDiameterModelsPreferred10x100 %>% group_by(responseVariable, species) %>% slice_max(aucDeltaAicN, n = 1, with_ties = FALSE) %>% select(responseVariable, species, aicName, aicFitting),
                          by = join_by(responseVariable, species)),
                heightDiameterModelsPreferred10x100 %>% group_by(responseVariable, species) %>% slice_max(aucNse, n = 1, with_ties = FALSE) %>% select(responseVariable, species, nseName, nseFitting),
                by = join_by(responseVariable, species)) %>%
...
heightDiameterFitStats10x100 %>% filter(is.na(distinct) | (distinct == TRUE)) %>%
...
heightDiameterFitStats10x100 %>% filter(is.na(distinct)) %>% select(fitSet, responseVariable, species, name, fitting) %>% arrange(desc(fitSet))

# summaries for supplemental material
standVariableSelection10x100 = heightDiameterPredictors10x100 %>% filter(distinct == 1, isBaseForm == FALSE) %>% 
  group_by(responseVariable, species, name, fitting) %>%
  summarize(hasBasalArea = hasBasalArea[1], hasRelative = hasRelative[1],
            .groups = "drop")
standVariableSelection10x100 %>% group_by(responseVariable, species) %>%
...
plot_auc_bank(heightFromDiameterModelComparison2x250, heightFromDiameterModelComparison5x100, heightFromDiameterModelComparison10x50, heightFromDiameterModelComparison5x100random, modelEfficiency = TRUE, legendHjustification = 1, rightPlotSpacer = 0.6)
#plot_auc_bank(heightFromDiameterModelComparison2x500, heightFromDiameterModelComparison5x200, heightFromDiameterModelComparison10x100, heightFromDiameterModelComparison5x200random, modelEfficiency = TRUE)
#ggsave("trees/height-diameter/figures/Figure S04 height AUCs Nash-Sutcliffe base.png", height = 20, width = 17, units = "cm", dpi = 150)
plot_auc_bank(heightFromDiameterModelComparison2x250, heightFromDiameterModelComparison5x100, heightFromDiameterModelComparison10x50, heightFromDiameterModelComparison5x100random, modelEfficiency = TRUE, generalized = TRUE, legendHjustification = 1, rightPlotSpacer = 0.15)
#plot_auc_bank(heightFromDiameterModelComparison2x500, heightFromDiameterModelComparison5x200, heightFromDiameterModelComparison10x100, heightFromDiameterModelComparison5x200random, modelEfficiencys = TRUE, generalized = TRUE)
#ggsave("trees/height-diameter/figures/Figure S04 height AUCs Nash-Sutcliffe generalized.png", height = 20, width = 17, units = "cm", dpi = 150)
...
diameterFromHeightModelComparison10x50 = get_model_comparison(heightDiameterModelRanking10x50, "DBH", heightDiameterModelRanking10x50)
...
#ggsave("trees/height-diameter/figures/Figure 03 diameter AUCs base.png", height = 18, width = 20, units = "cm", dpi = 150)
#ggsave("trees/height-diameter/figures/Figure 03 diameter AUCs base.tif", height = 18, width = 20, units = "cm", dpi = figureDpi, compression = "lzw+p")
#ggsave("trees/height-diameter/figures/Figure 03 diameter AUCs base", height = 18, width = 20, units = "cm", dpi = figureDpi, device = cairo_pdf, fallback_resolution = figureDpi)
plot_auc_bank(diameterFromHeightModelComparison2x250, diameterFromHeightModelComparison10x50, generalized = TRUE, legendHjustification = 1.02)
...
#ggsave("trees/height-diameter/figures/Figure 04 diameter AUCs generalized.png", height = 21, width = 20, units = "cm", dpi = 150)
#ggsave("trees/height-diameter/figures/Figure 04 diameter AUCs generalized.tif", height = 21, width = 20, units = "cm", dpi = figureDpi, compression = "lzw+p")
#ggsave("trees/height-diameter/figures/Figure 04 diameter AUCs generalized", height = 21, width = 20, units = "cm", dpi = figureDpi, device = cairo_pdf, fallback_resolution = figureDpi)
...
#ggsave("trees/height-diameter/figures/Figure S05 diameter AUCs 5x100 base.png", height = 18, width = 20, units = "cm", dpi = 150)
...
#ggsave("trees/height-diameter/figures/Figure S06 diameter AUCs 5x100 generalized.png", height = 21, width = 20, units = "cm", dpi = 150)

plot_auc_bank(diameterFromHeightModelComparison2x250, diameterFromHeightModelComparison5x100, diameterFromHeightModelComparison10x50, diameterFromHeightModelComparison5x100random, modelEfficiency = TRUE, legendHjustification = 1, rightPlotSpacer = 0.5)
#plot_auc_bank(diameterFromHeightModelComparison2x500, diameterFromHeightModelComparison5x200, diameterFromHeightModelComparison10x100, diameterFromHeightModelComparison5x200random, modelEfficiency = TRUE)
#ggsave("trees/height-diameter/figures/Figure S07 diameter AUCs bounds base.png", height = 18, width = 18, units = "cm", dpi = 150)
plot_auc_bank(diameterFromHeightModelComparison2x250, diameterFromHeightModelComparison5x100, diameterFromHeightModelComparison10x50, diameterFromHeightModelComparison5x100random, modelEfficiency = TRUE, generalized = TRUE, legendHjustification = 1, rightPlotSpacer = 0.05)
#plot_auc_bank(diameterFromHeightModelComparison2x500, diameterFromHeightModelComparison5x200, diameterFromHeightModelComparison10x100, diameterFromHeightModelComparison5x200random, modelEfficiency = TRUE, generalized = TRUE)
#ggsave("trees/height-diameter/figures/Figure S07 diameter AUCs bounds generalized.png", height = 21, width = 18, units = "cm", dpi = 150)
...
ggplot() +
  geom_segment(aes(x = 0.93, y = 0, xend = 0.93, yend = 0.2), color = "grey80", linetype = "longdash") +
  geom_histogram(aes(x = if_else(nse > -0.05, nse, NA_real_), y = after_stat(count / sum(count)), fill = species), heightDiameterFitStats10x50 %>% filter(responseVariable == "height", isBaseForm, distinct), binwidth = 0.01, na.rm = TRUE) +
  labs(x = NULL, y = "fraction of distinct\nmodel fits, %", fill = NULL, title = bquote(.(plotLetters[1])~"base form height prediction")) +
  #labs(x = NULL, y = "fraction of distinct\nmodel fits, %", fill = NULL, title = bquote(bold(.(plotLetters[1]))~"base form height prediction")) +
ggplot() +
  geom_segment(aes(x = 0.90, y = 0, xend = 0.90, yend = 0.2), color = "grey80", linetype = "longdash") +
  geom_histogram(aes(x = if_else(nse > -0.05, nse, NA_real_), y = after_stat(count / sum(count)), fill = species), heightDiameterFitStats10x50 %>% filter(responseVariable == "DBH", isBaseForm, distinct), binwidth = 0.01, na.rm = TRUE) +
...
ggplot() +
  geom_segment(aes(x = 0.93, y = 0, xend = 0.93, yend = 0.2), color = "grey80", linetype = "longdash") +
  geom_histogram(aes(x = if_else(nse > -0.05, nse, NA_real_), y = after_stat(count / sum(count)), fill = species), heightDiameterFitStats10x50 %>% filter(responseVariable == "height", isBaseForm == FALSE, distinct), binwidth = 0.01, na.rm = TRUE) +
  labs(x = "model efficiency", y = "fraction of distinct\nmodel fits, %", fill = NULL, title = bquote(.(plotLetters[3])~"generalized height prediction")) +
  #labs(x = "model efficiency", y = "fraction of distinct\nmodel fits, %", fill = NULL, title = bquote(bold(.(plotLetters[3]))~"generalized height prediction")) +
ggplot() +
  geom_segment(aes(x = 0.90, y = 0, xend = 0.90, yend = 0.2), color = "grey80", linetype = "longdash") +
  geom_histogram(aes(x = if_else(nse > -0.05, nse, NA_real_), y = after_stat(count / sum(count)), fill = species), heightDiameterFitStats10x50 %>% filter(responseVariable == "DBH", isBaseForm == FALSE, distinct), binwidth = 0.01, na.rm = TRUE) +
...
plot_layout(nrow = 2, ncol = 2, guides = "collect") &
  ...
  scale_x_continuous(breaks = seq(0, 1, by = 0.2), labels = scales::label_percent()) &
...
## Figure 6: fitting success, model distinctness, and predictor variable use
# TODO: how to include generalizing variable selection in GAMs and random forests?
modelFitStats2x500 = heightDiameterFitStats2x500 %>% group_by(responseVariable, species, name, fitting) %>%
  summarize(modelFit = is.na(distinct[1]) == FALSE, .groups = "drop_last") %>% 
  group_by(responseVariable, species) %>%
  summarize(fitPct = 100 * mean(modelFit), .groups = "drop") %>%
  mutate(species = factor(species, levels = levels(heightDiameterPredictors2x250$species)))

ggplot() +
  geom_col(aes(x = fitPct, y = species, fill = species), modelFitStats2x500 %>% filter(responseVariable == "height"), width = 0.6) + 
  ...
  scale_y_discrete(limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_col(aes(x = distinctPct, y = species, fill = species), heightDiameterPredictors2x250 %>% filter(responseVariable == "height") %>% group_by(species) %>% summarize(distinctPct = 100 * sum(distinct) / n()), width = 0.6) + 
  coord_cartesian(xlim = c(0, 100)) +
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_violin(aes(x = meanAbsolutePercentPlantationEffect, y = species, color = species, group = species), heightDiameterFitStats2x500 %>% filter(responseVariable == "height", distinct), quantiles = c(0.25, 0.5, 0.75), quantile.linetype = "solid", na.rm = TRUE, width = 1.2) +
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_count(aes(x = nBasalArea * distinct, y = species, color = species), heightDiameterPredictors2x250 %>% filter(responseVariable == "height", nBasalArea > 0)) + 
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_count(aes(x = nPhysio * distinct, y = species, color = species), heightDiameterPredictors2x250 %>% filter(responseVariable == "height", nPhysio > 0), shape = 18) + 
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_count(aes(x = nRelative * distinct, y = species, color = species), heightDiameterPredictors2x250 %>% filter(responseVariable == "height", nRelative > 0)) + 
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot(modelFitStats2x500 %>% filter(responseVariable == "DBH")) +
  geom_col(aes(x = fitPct, y = species, fill = species), width = 0.6) + 
  coord_cartesian(xlim = c(0, 100)) +
  labs(x = "model forms\nfittable, %", y = NULL, fill = NULL, title = bquote(.(plotLetters[7])~"DBH fits")) +
  #labs(x = "model forms\nfittable, %", y = NULL, fill = NULL, title = bquote(bold(.(plotLetters[7]))~"DBH fits")) +
  scale_y_discrete(limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_col(aes(x = distinctPct, y = species, fill = species), width = 0.6, heightDiameterPredictors2x250 %>% filter(responseVariable == "DBH") %>% group_by(species) %>% summarize(distinctPct = 100 * sum(distinct) / n())) + 
  coord_cartesian(xlim = c(0, 100)) +
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_violin(aes(x = meanAbsolutePercentPlantationEffect, y = species, color = species, group = species), heightDiameterFitStats2x500 %>% filter(responseVariable == "DBH", distinct), quantiles = c(0.25, 0.5, 0.75), quantile.linetype = "solid", na.rm = TRUE, width = 1.2) +
  coord_cartesian(xlim = c(0, 60)) +
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_count(aes(x = nBasalArea * distinct, y = species, color = species), heightDiameterPredictors2x250 %>% filter(responseVariable == "DBH", nBasalArea > 0)) + 
  coord_cartesian(xlim = c(-0.2, 2.2)) +
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_count(aes(x = nPhysio * distinct, y = species, color = species), heightDiameterPredictors2x250 %>% filter(responseVariable == "DBH", nPhysio > 0), shape = 18) + 
  coord_cartesian(xlim = c(-0.3, 5.3)) +
  ...
  scale_y_discrete(labels = NULL, limits = rev(levels(heightDiameterPredictors2x250$species))) +
ggplot() +
  geom_count(aes(x = nRelative * distinct, y = species, color = species), heightDiameterPredictors2x250 %>% filter(responseVariable == "DBH", nRelative > 0)) + 
...
plot_layout(nrow = 2, ncol = 6, widths = c(1.7, 1.7, 2.2, 1.7, 2.7, 1.2), guides = "collect") &
  scale_color_manual(breaks = levels(heightDiameterPredictors2x250$species), limits = levels(heightDiameterPredictors2x250$species), values = speciesGroupColors) &
  scale_fill_manual(breaks = levels(heightDiameterPredictors2x250$species), limits = levels(heightDiameterPredictors2x250$species), values = speciesGroupColors) &
...
#psmeHeightTemesgen = 1.3 + exp(5.7567 - 6.7792*psme2022$dbh^-0.2795)
psmeHeightWest = predict(psmeElliottHeight$gam, psme2022 %>% rename(DBH = dbh))
psmeDbhWest = predict(psmeElliottDbh$gam, psme2022 %>% rename(TotalHt = height))
psmeReference = bind_rows(bind_rows(psmeHeightFromDiameterPreferred$ratkowsky$stats %>% mutate(model = "base form 1"),
                                    psmeHeightFromDiameterPreferred$hossfeld$stats %>% mutate(model = "base form 2"),
                                    psmeHeightFromDiameterPreferred$sharmaPartonBalPhysio$stats %>% mutate(model = "generalized height"),
                                    #get_fit_statistics("Temesgen et al. 2007", "nls", "height", psme2022, psmeHeightTemesgen, estimatedParameters = 3, psme2022, psmeHeightTemesgen) %>% mutate(model = "previous model"),
                                    get_fit_statistics("West and Strimbu 2025", "gam", "height", psme2022, psmeHeightWest, estimatedParameters = psmeElliottHeight$gam$stats$effectiveDegreesOfFreedom, psme2022, psmeHeightWest) %>% mutate(model = "previous model")) %>% 
                            mutate(responseVariable = "height"),
                          bind_rows(psmeDiameterFromHeightPreferred$ruark$stats %>% mutate(model = "base form 1"),
                                    psmeDiameterFromHeightPreferred$michaelisMentenReplace$stats %>% mutate(model = "base form 2"),
                                    ...
                                    get_fit_statistics("West and Strimbu 2025", "gam", "DBH", psme2022, psmeDbhWest, estimatedParameters = psmeElliottDbh$gam$stats$effectiveDegreesOfFreedom, psme2022, psmeDbhWest) %>% mutate(model = "previous model")) %>% 
...
abgrHeightLarsen = 1.37 + 0.3048 * exp(6.74974 - 5.49823 * (abgr2022$dbh / 2.54)^-0.327093)
abgrDbhLarsen = (-1/5.49823 * (log(3.28084 * (abgr2022$height - 1.37)) + 5.49823))^(1/-0.327093) # all NaNs from outer fractional power
abgrReference = bind_rows(bind_rows(abgrHeightFromDiameterPreferred$chapmanRichards$stats %>% mutate(model = "base form 1"),
                                    abgrHeightFromDiameterPreferred$michaelisMenten$stats %>% mutate(model = "base form 2"),
                                    abgrHeightFromDiameterPreferred$chapmanRichardsBal$stats %>% mutate(model = "generalized height"),
                                    get_fit_statistics("Larsen et al. 1999", "nls", "height", abgr2022, abgrHeightLarsen, estimatedParameters = 4, abgr2022, abgrHeightLarsen) %>% mutate(model = "previous model")) %>% 
                            mutate(responseVariable = "height"),
                          bind_rows(abgrDiameterFromHeightPreferred$linear$stats %>% mutate(model = "base form 1"),
                                    abgrDiameterFromHeightPreferred$ruark$stats %>% mutate(model = "base form 2"),
                                    abgrDiameterFromHeightPreferred$powerAbat$stats %>% mutate(model = "generalized DBH"),
                                    get_fit_statistics("Larsen et al. 1999", "nls", "DBH", abgr2022, abgrDbhLarsen, estimatedParameters = 4, abgr2022, abgrDbhLarsen) %>% mutate(model = "previous model")) %>% 
                            mutate(responseVariable = "DBH")) %>%
...
ggplot() +
  ...
  geom_line(aes(x = dbh, y = predict(psmeHeightFromDiameterPreferred$hossfeld), color = "Hossfeld IV (MAE, ΔAICn)", group = isPlantation, linetype = isPlantation), psme2022) +
  ...
  scale_color_manual(breaks = c("Ratkowsky (MAE, RMSE, ME)", "Hossfeld IV (MAE, ΔAICn)", "Sharma-Parton BA+L RelDbh physio", "Temesgen et al. 2007"), values = c("dodgerblue2", "red2", "green2", "grey70")) +
...
ggplot() +
  ...
  geom_line(aes(x = predict(psmeDiameterFromHeightPreferred$michaelisMentenReplace), y = height, color = "Michaelis-Menten replace (MAE)", group = isPlantation, linetype = isPlantation), psme2022) +
  coord_cartesian(xlim = c(0, 250), ylim = c(0, 75)) +
  guides(color = guide_legend(position = "inside", override.aes = list(alpha = 0.8)), linetype = "none") +
  ...
  scale_color_manual(breaks = c("Ruark (RMSE, ΔAICn, ME)", "Michaelis-Menten replace (MAE)", "Ruark ABA+T physio", "previous model"), values = c("dodgerblue2", "red2", "cyan", "grey70")) +
...
ggplot() +
  ...
  geom_line(aes(x = dbh, y = predict(abgrHeightFromDiameterPreferred$chapmanRichards), color = "Chapman-Richards (MAE, RMSE, ME)", group = isPlantation, linetype = isPlantation), abgr2022) +
  geom_line(aes(x = dbh, y = predict(abgrHeightFromDiameterPreferred$michaelisMenten), color = "Michaelis-Menten (ΔAICn)", group = isPlantation, linetype = isPlantation), abgr2022) +
  coord_cartesian(xlim = c(0, 250), ylim = c(0, 75)) +
  guides(color = guide_legend(position = "inside", override.aes = list(alpha = 0.8)), linetype = "none") +
  ...
  scale_color_manual(breaks = c("Chapman-Richards (MAE, RMSE, ME)", "Michaelis-Menten (ΔAICn)", "Chapman-Richards BA+L", "previous model"), values = c("dodgerblue2", "red2", "green2", "grey70")) +
  theme(legend.justification = c(1, 0), legend.position.inside = c(1, 0.01)) +
ggplot() +
  geom_point(aes(x = dbh, y = height), abgr2022, alpha = 0.08, color = "grey25", na.rm = TRUE, shape = 16, size = 1.2) +
  geom_line(aes(x = dbh, y = 1.37 + 0.3048 * exp(7.232880669 - 5.746899904 * (0.393701 * dbh)^-0.271564741), linetype = isPlantation), abgrReferenceDbh, color = "grey70") +
  ...
  geom_line(aes(x = predict(abgrDiameterFromHeightPreferred$schnute), y = height, color = "Ruark (MAE)", group = isPlantation, linetype = isPlantation), abgr2022) +
  coord_cartesian(xlim = c(0, 250), ylim = c(0, 75)) +
  guides(color = guide_legend(position = "inside", override.aes = list(alpha = 0.8)), linetype = "none") +
  ...
  scale_color_manual(breaks = c("linear (RMSE, ME)", "Ruark (MAE)", "power ABA+T", "previous model"), values = c("dodgerblue2", "red2", "cyan", "grey70")) +
  theme(legend.justification = c(1, 0), legend.position.inside = c(1, 0.01)) +
...
plot_layout(design = "12\n34\n55", heights = c(1, 1, 0)) &
  ...
  theme(legend.margin = margin(), legend.key.height = unit(0.85, "line"))
...
acmaHeightWang = 1.37 + 0.3048 * exp(5.21462 - 2.70252 * (0.393701 * acma2022$dbh)^-0.354756)
acmaDbhWang = 1/0.393701 * (1/2.70252 * (5.21462 - log(1/1.37 * (acma2022$height - 1.37))))^(1/-0.354756)
acmaHeightWest = predict(acmaElliottHeight$ratkowsky, acma2022 %>% rename(DBH = dbh))
acmaDbhWest = predict(acmaElliottDbh$gam, acma2022 %>% rename(TotalHt = height))
acmaReference = bind_rows(bind_rows(acmaHeightFromDiameterPreferred$weibull$stats %>% mutate(model = "base form 1"),
                                    acmaHeightFromDiameterPreferred$michaelisMenten$stats %>% mutate(model = "base form 1"),
                                    acmaHeightFromDiameterPreferred$michaelisMentenBal$stats %>% mutate(model = "generalized height"),
                                    #get_fit_statistics("Wang and Hann 1988", "nls", "height", acma2022, acmaHeightWang, estimatedParameters = 4, acma2022, acmaHeightWang) %>% mutate(model = "previous model"),
                                    get_fit_statistics("West and Strimbu 2025", "gsl_nls", "height", acma2022, acmaHeightWest, estimatedParameters = acmaElliottHeight$ratkowsky$stats$effectiveDegreesOfFreedom, acma2022, acmaHeightWest) %>% mutate(model = "previous model")) %>% 
                            mutate(responseVariable = "height"),
                          bind_rows(acmaDiameterFromHeightPreferred$randomForest$stats %>% mutate(model = "base form 1"),
                                    ...
                                    #get_fit_statistics("Wang and Hann 1988", "nls", "DBH", acma2022, acmaDbhWang, estimatedParameters = 4, acma2022, acmaDbhWang) %>% mutate(model = "previous model"),
                                    get_fit_statistics("West and Strimbu 2025", "gam", "DBH", acma2022, acmaDbhWest, estimatedParameters = acmaElliottDbh$gam$stats$effectiveDegreesOfFreedom, acma2022, acmaDbhWest) %>% mutate(model = "previous model")) %>% 
                            mutate(responseVariable = "DBH")) %>%
...
ggplot() +
  ...
  geom_line(aes(x = dbh, y = predict(acmaHeightFromDiameterPreferred$michaelisMenten), color = "Michaelis-Menten (MAE, ΔAICn)", group = isPlantation, linetype = isPlantation), acma2022) +
  geom_line(aes(x = dbh, y = predict(acmaHeightFromDiameterPreferred$weibull), color = "Weibull (RMSE, ME)", group = isPlantation, linetype = isPlantation), acma2022) +
  coord_cartesian(xlim = c(0, 250), ylim = c(0, 75)) +
  guides(color = guide_legend(position = "inside", override.aes = list(alpha = 0.8)), linetype = "none") +
  ...
  scale_color_manual(breaks = c("Weibull (RMSE, ME)", "Michaelis-Menten (MAE, ΔAICn)", "Michaelis-Menten BA+L", "previous model"), values = c("dodgerblue2", "red2", "green2", "grey70")) +
  theme(legend.justification = c(1, 1), legend.position.inside = c(1, 1.03)) +
ggplot() +
  geom_point(aes(x = dbh, y = height), acma2022, alpha = 0.08, color = "grey25", na.rm = TRUE, shape = 16, size = 1.2) +
  #geom_line(aes(x = acmaReferenceDbh, y = 1.37 + 0.3048 * exp(5.21462 - 2.70252 * (0.393701 * acmaReferenceDbh)^-0.354756), linetype = "previous model"), acma2022, color = "grey70") + # Wang and Hann 1988
  ...
  geom_line(aes(x = predict(acmaDiameterFromHeightPreferred$randomForest, acma2022)$predictions, y = height, color = "random forest (RMSE, ME)", group = isPlantation, linetype = isPlantation), acma2022, orientation = "y") +
  geom_line(aes(x = predict(acmaDiameterFromHeightPreferred$gam), y = height, color = "REML GAM (MAE)", group = isPlantation, linetype = isPlantation), acma2022, orientation = "y") +
  coord_cartesian(xlim = c(0, 250), ylim = c(0, 75)) +
  guides(color = guide_legend(position = "inside", override.aes = list(alpha = 0.8)), linetype = "none") +
...
ggplot() +
  ...
  geom_line(aes(x = dbh, y = predict(otherHeightFromDiameterPreferred$sibbesen), color = "Sibbesen (RMSE, ME)", group = isPlantation, linetype = isPlantation), other2022) +
  geom_line(aes(x = dbh, y = predict(otherHeightFromDiameterPreferred$korf), color = "Korf (MAE, ΔAICn)", group = isPlantation, linetype = isPlantation), other2022) +
  coord_cartesian(xlim = c(0, 250), ylim = c(0, 75)) +
  guides(color = guide_legend(position = "inside", override.aes = list(alpha = 0.8)), linetype = "none") +
  ...
  scale_color_manual(breaks = c("Sibbesen (RMSE, ME)", "Korf (MAE, ΔAICn)", "Sharma-Parton", "previous model"), values = c("dodgerblue2", "red2", "green2", "grey70")) +
  theme(legend.justification = c(1, 1), legend.position.inside = c(1, 1.03)) +
ggplot() +
  geom_point(aes(x = dbh, y = height), other2022, alpha = 0.08, color = "grey25", na.rm = TRUE, shape = 16, size = 1.2) +
  ...
  geom_line(aes(x = predict(otherDiameterFromHeightPreferred$gam), y = height, color = "REML GAM (MAE, RMSE, ΔAICn, ME)", group = isPlantation, linetype = isPlantation), other2022, orientation = "y") +
...
selectedModels = bind_rows(psmeReference, abgrReference, acmaReference, otherReference) %>% 
  mutate(pctOutOfRange = 100 * (validation$nPredictionNA + validation$nPredictionNegative + validation$nPredictionAboveMax) / validation$n,
...
selectedModels %>% select(species, name, validation)
selectedModels$validation$n
selectedModels$deltaAicN
...
ggplot() +
  geom_point(aes(x = validation$mae, y = species, color = model, shape = model, size = model), selectedModels %>% filter(responseVariable == "height"), alpha = 0.6) +
...
ggplot() +
  geom_point(aes(x = validation$rmse, y = species, color = model, shape = model, size = model), selectedModels %>% filter(responseVariable == "height"), alpha = 0.6) +
  coord_cartesian(xlim = c(0, NA)) +
  guides(color = "none", shape = "none", size = "none") +
...
ggplot() +
  ...
  scale_y_discrete(labels = NULL, limits = rev) +
ggplot() +
  geom_point(aes(x = 100 * validation$nse, y = species, color = model, shape = model, size = model), selectedModels %>% filter(responseVariable == "height"), alpha = 0.6) +
  coord_cartesian(xlim = c(0, 100)) +
  guides(color = "none", shape = "none", size = "none") +
  labs(x = "model efficiency", y = NULL, color = NULL, shape = NULL, size = NULL) +
  scale_y_discrete(labels = NULL, limits = rev) +
ggplot() +
  geom_point(aes(x = pctOutOfRange, y = species, color = model, shape = model, size = model), selectedModels %>% filter(responseVariable == "DBH"), alpha = 0.6, na.rm = TRUE) +
  coord_cartesian(xlim = c(0, 25)) +
  guides(color = "none", shape = "none", size = "none") +
  labs(x = "out of range, %", y = NULL, color = NULL, shape = NULL, size = NULL, title = paste0(plotLetters[2], " DBH prediction")) +
  scale_y_discrete(limits = rev) +
ggplot() +
  geom_point(aes(x = validation$mae, y = species, color = model, shape = model, size = model), selectedModels %>% filter(responseVariable == "DBH"), alpha = 0.6, na.rm = TRUE) +
  coord_cartesian(xlim = c(0, NA)) +
  guides(color = "none", shape = "none", size = "none") +
  labs(x = "MAE, cm", y = NULL, color = NULL, shape = NULL, size = NULL) +
  scale_y_discrete(labels = NULL, limits = rev) +
ggplot() +
  geom_point(aes(x = validation$rmse, y = species, color = model, shape = model, size = model), selectedModels %>% filter(responseVariable == "DBH"), alpha = 0.6, na.rm = TRUE) +
  coord_cartesian(xlim = c(0, NA)) +
  guides(color = "none", shape = "none", size = "none") +
  labs(x = "RMSE, cm", y = NULL, color = NULL, shape = NULL, size = NULL) +
  scale_y_discrete(labels = NULL, limits = rev) +
ggplot() +
  geom_point(aes(x = deltaAicN, y = species, color = model, shape = model, size = model), selectedModels %>% filter(responseVariable == "DBH"), alpha = 0.6, na.rm = TRUE) +
  coord_cartesian(xlim = c(0, 5)) +
  guides(color = "none", shape = "none", size = "none") +
  labs(x = "ΔAICn", y = NULL, color = NULL, shape = NULL, size = NULL) +
  scale_x_continuous(breaks = seq(0, 6)) +
  scale_y_discrete(labels = NULL, limits = rev) +
ggplot() +
  geom_point(aes(x = 100 * nse, y = species, color = model, shape = model, size = model), selectedModels %>% filter((responseVariable == "DBH") | ((species == "bigleaf maple") & (model == "generalized height"))) %>% mutate(nse = if_else(responseVariable == "DBH", validation$nse, -1)), alpha = 0.6, na.rm = TRUE) + # add fake height data point off plot to get ggplot2 3.5.1 to draw the generalized height legend key
...
rankedModelForms = bind_rows(heightDiameterModelRanking2x250,
                             heightDiameterModelRanking5x100,
